...
func groupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"group01", "group02", "group03", "package01", "package02", "package03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_GROUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_GROUP_ENTID"])
