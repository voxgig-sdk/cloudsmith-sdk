...
func TestDockerDirect(t *testing.T) {
	t.Run("direct-list-docker", func(t *testing.T) {
		setup := dockerDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-docker", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-docker", func(t *testing.T) {
		setup := dockerDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-docker", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/docker/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type dockerDirectSetupResult struct {
...
func dockerDirectSetup(mockres any) *dockerDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_DOCKER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_DOCKER_ENTID"]; ok {
		...
		return &dockerDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &dockerDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
