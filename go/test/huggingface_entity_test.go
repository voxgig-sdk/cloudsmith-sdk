...
func TestHuggingfaceEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Huggingface(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Huggingface(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		huggingfaceRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "huggingface"}), "huggingface_ref01"))
		...
		if huggingfaceRef01Data["id"] == nil {
		...
		if huggingfaceRef01ResdataUp0["id"] != huggingfaceRef01DataUp0Up["id"] {
		...
		huggingfaceRef01DataDt0LoadResult := core.ToMapAny(entityData(huggingfaceRef01DataDt0Loaded))
		if huggingfaceRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if huggingfaceRef01DataDt0LoadResult["id"] != huggingfaceRef01Data["id"] {
