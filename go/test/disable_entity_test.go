...
func TestDisableEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		disableRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.disable")))
...
func disableBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"disable01", "disable02", "disable03", "entitlement01", "entitlement02", "entitlement03", "org01", "org02", "org03", "repo01", "repo02", "repo03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_DISABLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_DISABLE_ENTID"])
