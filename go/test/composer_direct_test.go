...
func TestComposerDirect(t *testing.T) {
	t.Run("direct-list-composer", func(t *testing.T) {
		setup := composerDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-composer", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-composer", func(t *testing.T) {
		setup := composerDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-composer", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/composer/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type composerDirectSetupResult struct {
...
func composerDirectSetup(mockres any) *composerDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_COMPOSER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_COMPOSER_ENTID"]; ok {
		...
		return &composerDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &composerDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
