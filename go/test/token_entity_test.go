...
func TestTokenEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_TOKEN_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		tokenRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.token", setup.data)))
		var tokenRef01Data map[string]any
		if len(tokenRef01DataRaw) > 0 {
			tokenRef01Data = core.ToMapAny(tokenRef01DataRaw[0][1])
		}
...
func tokenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_TOKEN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_TOKEN_ENTID"])
