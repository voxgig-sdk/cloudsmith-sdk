...
func TestRepositoryWebhookDirect(t *testing.T) {
	t.Run("direct-list-repository_webhook", func(t *testing.T) {
		setup := repository_webhookDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-repository_webhook", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-repository_webhook", func(t *testing.T) {
		setup := repository_webhookDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-repository_webhook", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["repo"] = setup.idmap["repo01"]
...
type repository_webhookDirectSetupResult struct {
...
func repository_webhookDirectSetup(mockres any) *repository_webhookDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_REPOSITORY_WEBHOOK_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_REPOSITORY_WEBHOOK_ENTID"]; ok {
		...
		return &repository_webhookDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &repository_webhookDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
