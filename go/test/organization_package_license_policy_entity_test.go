...
func TestOrganizationPackageLicensePolicyEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.OrganizationPackageLicensePolicy(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.OrganizationPackageLicensePolicy(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		organizationPackageLicensePolicyRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "organization_package_license_policy"}), "organization_package_license_policy_ref01"))
