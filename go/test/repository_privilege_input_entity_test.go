...
func TestRepositoryPrivilegeInputEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.RepositoryPrivilegeInput(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.RepositoryPrivilegeInput(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		repositoryPrivilegeInputRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.repository_privilege_input")))
