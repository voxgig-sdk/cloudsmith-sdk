...
func TestHuggingfaceDirect(t *testing.T) {
	t.Run("direct-list-huggingface", func(t *testing.T) {
		setup := huggingfaceDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-huggingface", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/huggingface",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-huggingface", func(t *testing.T) {
		setup := huggingfaceDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-huggingface", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/huggingface/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type huggingfaceDirectSetupResult struct {
...
func huggingfaceDirectSetup(mockres any) *huggingfaceDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_HUGGINGFACE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_HUGGINGFACE_ENTID"]; ok {
		...
		return &huggingfaceDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &huggingfaceDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
