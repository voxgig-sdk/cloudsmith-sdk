...
func TestTestEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_TEST_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		testRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.test", setup.data)))
		var testRef01Data map[string]any
		if len(testRef01DataRaw) > 0 {
			testRef01Data = core.ToMapAny(testRef01DataRaw[0][1])
		}
...
func testBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"test01", "test02", "test03", "repo01", "repo02", "repo03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_TEST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_TEST_ENTID"])
