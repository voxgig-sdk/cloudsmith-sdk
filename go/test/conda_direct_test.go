...
func TestCondaDirect(t *testing.T) {
	t.Run("direct-list-conda", func(t *testing.T) {
		setup := condaDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-conda", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-conda", func(t *testing.T) {
		setup := condaDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-conda", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/conda/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type condaDirectSetupResult struct {
...
func condaDirectSetup(mockres any) *condaDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_CONDA_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_CONDA_ENTID"]; ok {
		...
		return &condaDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &condaDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
