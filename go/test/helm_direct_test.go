...
func TestHelmDirect(t *testing.T) {
	t.Run("direct-list-helm", func(t *testing.T) {
		setup := helmDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-helm", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/helm",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-helm", func(t *testing.T) {
		setup := helmDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-helm", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/helm/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type helmDirectSetupResult struct {
...
func helmDirectSetup(mockres any) *helmDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_HELM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_HELM_ENTID"]; ok {
		...
		return &helmDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &helmDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
