...
func TestDebDirect(t *testing.T) {
	t.Run("direct-list-deb", func(t *testing.T) {
		setup := debDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-deb", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-deb", func(t *testing.T) {
		setup := debDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-deb", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/deb/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type debDirectSetupResult struct {
...
func debDirectSetup(mockres any) *debDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_DEB_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_DEB_ENTID"]; ok {
		...
		return &debDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &debDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
