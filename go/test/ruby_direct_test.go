...
func TestRubyDirect(t *testing.T) {
	t.Run("direct-list-ruby", func(t *testing.T) {
		setup := rubyDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-ruby", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/ruby",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-ruby", func(t *testing.T) {
		setup := rubyDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-ruby", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/ruby/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type rubyDirectSetupResult struct {
...
func rubyDirectSetup(mockres any) *rubyDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_RUBY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_RUBY_ENTID"]; ok {
		...
		return &rubyDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &rubyDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
