...
func TestNpmDirect(t *testing.T) {
	t.Run("direct-list-npm", func(t *testing.T) {
		setup := npmDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-npm", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/npm",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-npm", func(t *testing.T) {
		setup := npmDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-npm", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/npm/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type npmDirectSetupResult struct {
...
func npmDirectSetup(mockres any) *npmDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_NPM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_NPM_ENTID"]; ok {
		...
		return &npmDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &npmDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
