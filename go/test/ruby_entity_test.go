...
func TestRubyEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Ruby(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Ruby(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		rubyRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "ruby"}), "ruby_ref01"))
		...
		rubyRef01Data = core.ToMapAny(entityData(rubyRef01DataResult))
		if rubyRef01Data == nil {
			t.Fatal("expected create result to be a map")
		}
		if rubyRef01Data["id"] == nil {
		...
		rubyRef01ResdataUp0 := core.ToMapAny(entityData(rubyRef01ResdataUp0Result))
		if rubyRef01ResdataUp0 == nil {
			t.Fatal("expected update result to be a map")
		}
		if rubyRef01ResdataUp0["id"] != rubyRef01DataUp0Up["id"] {
		...
		rubyRef01DataDt0LoadResult := core.ToMapAny(entityData(rubyRef01DataDt0Loaded))
		if rubyRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if rubyRef01DataDt0LoadResult["id"] != rubyRef01Data["id"] {
