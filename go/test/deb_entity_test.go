...
func TestDebEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.Deb(nil)
		if ent == nil {
			t.Fatal("expected non-nil DebEntity")
		}
	})
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Deb(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Deb(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		for _, _op := range []string{"create", "list", "update", "load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "deb." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_DEB_ENTID JSON to run live")
			return
		}
		client := setup.client

		// CREATE
		debRef01Ent := client.Deb(nil)
		debRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "deb"}, setup.data), "deb_ref01"))
		debRef01Data["identifier"] = setup.idmap["identifier01"]
		debRef01Data["owner"] = setup.idmap["owner01"]

		debRef01DataResult, err := debRef01Ent.Create(debRef01Data, nil)
		if err != nil {
		...
		debRef01ListResult, err := debRef01Ent.List(debRef01Match, nil)
		if err != nil {
		...
		debRef01MarkdefUp0Name := "auth_mode"
		debRef01MarkdefUp0Value := fmt.Sprintf("Mark01-deb_ref01_%d", setup.now)
		debRef01DataUp0Up[debRef01MarkdefUp0Name] = debRef01MarkdefUp0Value

		debRef01ResdataUp0Result, err := debRef01Ent.Update(debRef01DataUp0Up, nil)
		if err != nil {
...
func debBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "deb", "DebTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read deb test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse deb test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_DEB_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_DEB_ENTID"])
