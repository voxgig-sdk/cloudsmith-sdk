...
func TestPackageDirect(t *testing.T) {
	t.Run("direct-list-package", func(t *testing.T) {
		setup := packageDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-package", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-package", func(t *testing.T) {
		setup := packageDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-package", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type packageDirectSetupResult struct {
...
func packageDirectSetup(mockres any) *packageDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_PACKAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_PACKAGE_ENTID"]; ok {
		...
		return &packageDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &packageDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
