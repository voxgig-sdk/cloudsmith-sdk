...
func TestCranDirect(t *testing.T) {
	t.Run("direct-list-cran", func(t *testing.T) {
		setup := cranDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-cran", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-cran", func(t *testing.T) {
		setup := cranDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-cran", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/cran/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type cranDirectSetupResult struct {
...
func cranDirectSetup(mockres any) *cranDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_CRAN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_CRAN_ENTID"]; ok {
		...
		return &cranDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &cranDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
