...
func TestSwiftDirect(t *testing.T) {
	t.Run("direct-list-swift", func(t *testing.T) {
		setup := swiftDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-swift", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/swift",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-swift", func(t *testing.T) {
		setup := swiftDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-swift", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/swift/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type swiftDirectSetupResult struct {
...
func swiftDirectSetup(mockres any) *swiftDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_SWIFT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_SWIFT_ENTID"]; ok {
		...
		return &swiftDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &swiftDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
