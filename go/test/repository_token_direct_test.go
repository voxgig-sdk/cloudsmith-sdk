...
func TestRepositoryTokenDirect(t *testing.T) {
	t.Run("direct-list-repository_token", func(t *testing.T) {
		setup := repository_tokenDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-repository_token", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-repository_token", func(t *testing.T) {
		setup := repository_tokenDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-repository_token", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type repository_tokenDirectSetupResult struct {
...
func repository_tokenDirectSetup(mockres any) *repository_tokenDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_REPOSITORY_TOKEN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_REPOSITORY_TOKEN_ENTID"]; ok {
		...
		return &repository_tokenDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &repository_tokenDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
