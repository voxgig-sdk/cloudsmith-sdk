...
func TestGonDirect(t *testing.T) {
	t.Run("direct-list-gon", func(t *testing.T) {
		setup := gonDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-gon", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-gon", func(t *testing.T) {
		setup := gonDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-gon", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			params["identifier"] = "direct01"
			params["owner"] = "direct02"
			params["slug_perm"] = "direct03"
		}

		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/go/{slug_perm}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type gonDirectSetupResult struct {
...
func gonDirectSetup(mockres any) *gonDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_GON_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_GON_ENTID"]; ok {
		...
		return &gonDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &gonDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
