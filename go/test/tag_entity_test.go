...
func tagBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"tag01", "tag02", "tag03", "package01", "package02", "package03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_TAG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_TAG_ENTID"])
