...
func TestHexDirect(t *testing.T) {
	t.Run("direct-list-hex", func(t *testing.T) {
		setup := hexDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-hex", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/hex",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-hex", func(t *testing.T) {
		setup := hexDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-hex", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/hex/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type hexDirectSetupResult struct {
...
func hexDirectSetup(mockres any) *hexDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_HEX_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_HEX_ENTID"]; ok {
		...
		return &hexDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &hexDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
