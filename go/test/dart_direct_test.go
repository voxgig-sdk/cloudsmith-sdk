...
func TestDartDirect(t *testing.T) {
	t.Run("direct-list-dart", func(t *testing.T) {
		setup := dartDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-dart", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-dart", func(t *testing.T) {
		setup := dartDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-dart", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/dart/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type dartDirectSetupResult struct {
...
func dartDirectSetup(mockres any) *dartDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_DART_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_DART_ENTID"]; ok {
		...
		return &dartDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &dartDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
