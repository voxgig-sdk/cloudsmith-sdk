...
func TestPythonDirect(t *testing.T) {
	t.Run("direct-list-python", func(t *testing.T) {
		setup := pythonDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-python", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/python",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-python", func(t *testing.T) {
		setup := pythonDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-python", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/python/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type pythonDirectSetupResult struct {
...
func pythonDirectSetup(mockres any) *pythonDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_PYTHON_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_PYTHON_ENTID"]; ok {
		...
		return &pythonDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &pythonDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
