...
func TestRpmDirect(t *testing.T) {
	t.Run("direct-list-rpm", func(t *testing.T) {
		setup := rpmDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-rpm", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/rpm",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-rpm", func(t *testing.T) {
		setup := rpmDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-rpm", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/rpm/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type rpmDirectSetupResult struct {
...
func rpmDirectSetup(mockres any) *rpmDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_RPM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_RPM_ENTID"]; ok {
		...
		return &rpmDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &rpmDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
