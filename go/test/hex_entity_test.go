...
func TestHexEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Hex(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Hex(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		hexRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "hex"}), "hex_ref01"))
		...
		if hexRef01ResdataUp0["id"] != hexRef01DataUp0Up["id"] {
		...
		hexRef01DataDt0LoadResult := core.ToMapAny(entityData(hexRef01DataDt0Loaded))
		if hexRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if hexRef01DataDt0LoadResult["id"] != hexRef01Data["id"] {
