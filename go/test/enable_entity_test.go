...
func TestEnableEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		enableRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.enable")))
...
func enableBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"enable01", "enable02", "enable03", "entitlement01", "entitlement02", "entitlement03", "org01", "org02", "org03", "repo01", "repo02", "repo03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_ENABLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CLOUDSMITH_TEST_ENABLE_ENTID"])
