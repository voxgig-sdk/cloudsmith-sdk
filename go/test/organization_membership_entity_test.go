...
func TestOrganizationMembershipEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.OrganizationMembership(nil)
		if ent == nil {
			t.Fatal("expected non-nil OrganizationMembershipEntity")
		}
	})
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.OrganizationMembership(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.OrganizationMembership(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		setup := organization_membershipBasicSetup(nil)
		...
		for _, _op := range []string{"list", "update", "load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "organization_membership." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_MEMBERSHIP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_ = organizationMembershipRef01Data

		// LIST
		organizationMembershipRef01Ent := client.OrganizationMembership(nil)
		organizationMembershipRef01Match := map[string]any{
			"org_id": setup.idmap["org01"],
		}

		organizationMembershipRef01ListResult, err := organizationMembershipRef01Ent.List(organizationMembershipRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, organizationMembershipRef01ListOk := organizationMembershipRef01ListResult.([]any)
		if !organizationMembershipRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", organizationMembershipRef01ListResult)
		}

		// UPDATE
		organizationMembershipRef01DataUp0Up := map[string]any{
		...
		organizationMembershipRef01ResdataUp0 := core.ToMapAny(organizationMembershipRef01ResdataUp0Result)
		...
		// LOAD
		organizationMembershipRef01MatchDt0 := map[string]any{}
		organizationMembershipRef01DataDt0Loaded, err := organizationMembershipRef01Ent.Load(organizationMembershipRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		if organizationMembershipRef01DataDt0Loaded == nil {
...
func organization_membershipBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "organization_membership", "OrganizationMembershipTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read organization_membership test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse organization_membership test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("CLOUDSMITH_TEST_ORGANIZATION_MEMBERSHIP_ENTID")
