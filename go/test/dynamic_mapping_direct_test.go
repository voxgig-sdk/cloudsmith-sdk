...
func TestDynamicMappingDirect(t *testing.T) {
	t.Run("direct-list-dynamic_mapping", func(t *testing.T) {
		setup := dynamic_mappingDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-dynamic_mapping", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"org01", "provider_setting01"} {
	...
	t.Run("direct-load-dynamic_mapping", func(t *testing.T) {
		setup := dynamic_mappingDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-dynamic_mapping", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["provider_setting"] = setup.idmap["provider_setting01"]
...
type dynamic_mappingDirectSetupResult struct {
...
func dynamic_mappingDirectSetup(mockres any) *dynamic_mappingDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_DYNAMIC_MAPPING_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_DYNAMIC_MAPPING_ENTID"]; ok {
		...
		return &dynamic_mappingDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &dynamic_mappingDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
