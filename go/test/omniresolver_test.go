...
import (
	...
	omni "github.com/voxgig-sdk/cloudsmith-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CloudsmithError); is {
				out := map[string]any{
					"name":    "CloudsmithError",
...
func omniCtx(arg any, client *sdk.CloudsmithSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CloudsmithSDK); is {
			client = live
