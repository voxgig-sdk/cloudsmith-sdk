...
func TestCargoDirect(t *testing.T) {
	t.Run("direct-list-cargo", func(t *testing.T) {
		setup := cargoDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-cargo", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-cargo", func(t *testing.T) {
		setup := cargoDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-cargo", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/cargo/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type cargoDirectSetupResult struct {
...
func cargoDirectSetup(mockres any) *cargoDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_CARGO_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_CARGO_ENTID"]; ok {
		...
		return &cargoDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &cargoDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
