...
func TestMavenDirect(t *testing.T) {
	t.Run("direct-list-maven", func(t *testing.T) {
		setup := mavenDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-maven", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/maven",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-maven", func(t *testing.T) {
		setup := mavenDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-maven", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "repos/{owner}/{identifier}/upstream/maven/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type mavenDirectSetupResult struct {
...
func mavenDirectSetup(mockres any) *mavenDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_MAVEN_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_MAVEN_ENTID"]; ok {
		...
		return &mavenDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &mavenDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
