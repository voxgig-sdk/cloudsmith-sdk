...
func TestNugetDirect(t *testing.T) {
	t.Run("direct-list-nuget", func(t *testing.T) {
		setup := nugetDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-nuget", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-nuget", func(t *testing.T) {
		setup := nugetDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-nuget", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type nugetDirectSetupResult struct {
...
func nugetDirectSetup(mockres any) *nugetDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CLOUDSMITH_TEST_NUGET_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CLOUDSMITH_TEST_NUGET_ENTID"]; ok {
		...
		return &nugetDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &nugetDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
