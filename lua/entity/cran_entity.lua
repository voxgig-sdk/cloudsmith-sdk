...
local CranEntity = {}
CranEntity.__index = CranEntity


function CranEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CranEntity)
  self._name = "cran"
  self._client = client
...
function CranEntity:get_name()
  return self._name
end


function CranEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CranEntity.new(self._client, opts)
end
...
function CranEntity:mark_deleted()
  self._deleted = true
end


function CranEntity:deleted()
  return true == self._deleted
end


function CranEntity:data_set(args)
  if args ~= nil then
...
function CranEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CranEntity:match_set(args)
  if args ~= nil then
...
function CranEntity:stream(action, args, callopts)
...
---@param reqmatch CranLoadMatch
---@param ctrl? table
---@return Cran
---@return string? err
function CranEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CranListMatch
---@param ctrl? table
---@return Cran[]
---@return string? err
function CranEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CranCreateData
---@param ctrl? table
---@return Cran
---@return string? err
function CranEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata CranUpdateData
---@param ctrl? table
---@return Cran
---@return string? err
function CranEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function CranEntity:_run_op(ctx, post_done)
  local utility = self._utility
