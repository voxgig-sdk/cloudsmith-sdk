...
function TestEntity.new(client, entopts)
  entopts = entopts or {}
...
function TestEntity:get_name()
  return self._name
end


function TestEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TestEntity.new(self._client, opts)
end
...
function TestEntity:mark_deleted()
  self._deleted = true
end


function TestEntity:deleted()
  return true == self._deleted
end


function TestEntity:data_set(args)
  if args ~= nil then
...
function TestEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TestEntity:match_set(args)
  if args ~= nil then
...
function TestEntity:stream(action, args, callopts)
...
function TestEntity:_run_op(ctx, post_done)
  local utility = self._utility
