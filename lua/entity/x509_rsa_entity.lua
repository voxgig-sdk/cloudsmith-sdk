-- Cloudsmith SDK X509Rsa entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local X509RsaEntity = {}
X509RsaEntity.__index = X509RsaEntity


function X509RsaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, X509RsaEntity)
  self._name = "x509_rsa"
  self._client = client
...
function X509RsaEntity:get_name()
  return self._name
end


function X509RsaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return X509RsaEntity.new(self._client, opts)
end
...
function X509RsaEntity:mark_deleted()
  self._deleted = true
end


function X509RsaEntity:deleted()
  return true == self._deleted
end


function X509RsaEntity:data_set(args)
  if args ~= nil then
...
function X509RsaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function X509RsaEntity:match_set(args)
  if args ~= nil then
...
function X509RsaEntity:match_get()
...
function X509RsaEntity:stream(action, args, callopts)
...
function X509RsaEntity:_run_op(ctx, post_done)
  local utility = self._utility
