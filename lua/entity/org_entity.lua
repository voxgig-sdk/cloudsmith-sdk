...
local OrgEntity = {}
OrgEntity.__index = OrgEntity


function OrgEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrgEntity)
  self._name = "org"
  self._client = client
...
function OrgEntity:get_name()
  return self._name
end


function OrgEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrgEntity.new(self._client, opts)
end
...
function OrgEntity:mark_deleted()
  self._deleted = true
end


function OrgEntity:deleted()
  return true == self._deleted
end


function OrgEntity:data_set(args)
  if args ~= nil then
...
function OrgEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrgEntity:match_set(args)
  if args ~= nil then
...
function OrgEntity:stream(action, args, callopts)
...
---@param reqmatch OrgLoadMatch
---@param ctrl? table
---@return Org
---@return string? err
function OrgEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch OrgListMatch
---@param ctrl? table
---@return Org[]
---@return string? err
function OrgEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata OrgCreateData
---@param ctrl? table
---@return Org
---@return string? err
function OrgEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata OrgUpdateData
---@param ctrl? table
---@return Org
---@return string? err
function OrgEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch OrgRemoveMatch
---@param ctrl? table
---@return Org
---@return string? err
function OrgEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function OrgEntity:_run_op(ctx, post_done)
  local utility = self._utility
