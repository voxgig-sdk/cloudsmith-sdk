...
local DebEntity = {}
DebEntity.__index = DebEntity


function DebEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DebEntity)
  self._name = "deb"
  self._client = client
...
function DebEntity:get_name()
  return self._name
end


function DebEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DebEntity.new(self._client, opts)
end
...
function DebEntity:mark_deleted()
  self._deleted = true
end


function DebEntity:deleted()
  return true == self._deleted
end


function DebEntity:data_set(args)
  if args ~= nil then
...
function DebEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DebEntity:match_set(args)
  if args ~= nil then
...
function DebEntity:stream(action, args, callopts)
...
---@param reqmatch DebLoadMatch
---@param ctrl? table
---@return Deb
---@return string? err
function DebEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DebListMatch
---@param ctrl? table
---@return Deb[]
---@return string? err
function DebEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata DebCreateData
---@param ctrl? table
---@return Deb
---@return string? err
function DebEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata DebUpdateData
---@param ctrl? table
---@return Deb
---@return string? err
function DebEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function DebEntity:_run_op(ctx, post_done)
  local utility = self._utility
