-- Cloudsmith SDK Token entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TokenEntity = {}
TokenEntity.__index = TokenEntity


function TokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TokenEntity)
  self._name = "token"
  self._client = client
...
function TokenEntity:get_name()
  return self._name
end


function TokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TokenEntity.new(self._client, opts)
end
...
function TokenEntity:mark_deleted()
  self._deleted = true
end


function TokenEntity:deleted()
  return true == self._deleted
end


function TokenEntity:data_set(args)
  if args ~= nil then
...
function TokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TokenEntity:match_set(args)
  if args ~= nil then
...
function TokenEntity:match_get()
...
function TokenEntity:stream(action, args, callopts)
...
function TokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
