-- Cloudsmith SDK Gpg entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GpgEntity = {}
GpgEntity.__index = GpgEntity


function GpgEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GpgEntity)
  self._name = "gpg"
  self._client = client
...
function GpgEntity:get_name()
  return self._name
end


function GpgEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GpgEntity.new(self._client, opts)
end
...
function GpgEntity:mark_deleted()
  self._deleted = true
end


function GpgEntity:deleted()
  return true == self._deleted
end


function GpgEntity:data_set(args)
  if args ~= nil then
...
function GpgEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GpgEntity:match_set(args)
  if args ~= nil then
...
function GpgEntity:stream(action, args, callopts)
...
function GpgEntity:_run_op(ctx, post_done)
  local utility = self._utility
