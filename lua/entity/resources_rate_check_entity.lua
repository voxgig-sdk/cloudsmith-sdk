-- Cloudsmith SDK ResourcesRateCheck entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ResourcesRateCheckEntity = {}
ResourcesRateCheckEntity.__index = ResourcesRateCheckEntity


function ResourcesRateCheckEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResourcesRateCheckEntity)
  self._name = "resources_rate_check"
  self._client = client
...
function ResourcesRateCheckEntity:get_name()
  return self._name
end


function ResourcesRateCheckEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResourcesRateCheckEntity.new(self._client, opts)
end
...
function ResourcesRateCheckEntity:mark_deleted()
  self._deleted = true
end


function ResourcesRateCheckEntity:deleted()
  return true == self._deleted
end


function ResourcesRateCheckEntity:data_set(args)
  if args ~= nil then
...
function ResourcesRateCheckEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResourcesRateCheckEntity:match_set(args)
  if args ~= nil then
...
function ResourcesRateCheckEntity:match_get()
...
function ResourcesRateCheckEntity:stream(action, args, callopts)
...
---@param reqmatch ResourcesRateCheckLoadMatch
---@param ctrl? table
---@return ResourcesRateCheck
---@return string? err
function ResourcesRateCheckEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ResourcesRateCheckEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ResourcesRateCheckEntity
