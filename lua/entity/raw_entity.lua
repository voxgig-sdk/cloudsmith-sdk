-- Cloudsmith SDK Raw entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RawEntity = {}
RawEntity.__index = RawEntity


function RawEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RawEntity)
  self._name = "raw"
  self._client = client
...
function RawEntity:get_name()
  return self._name
end


function RawEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RawEntity.new(self._client, opts)
end
...
function RawEntity:mark_deleted()
  self._deleted = true
end


function RawEntity:deleted()
  return true == self._deleted
end


function RawEntity:data_set(args)
  if args ~= nil then
...
function RawEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RawEntity:match_set(args)
  if args ~= nil then
...
function RawEntity:stream(action, args, callopts)
...
function RawEntity:_run_op(ctx, post_done)
  local utility = self._utility
