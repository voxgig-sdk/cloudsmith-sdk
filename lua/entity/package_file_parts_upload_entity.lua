-- Cloudsmith SDK PackageFilePartsUpload entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PackageFilePartsUploadEntity = {}
PackageFilePartsUploadEntity.__index = PackageFilePartsUploadEntity


function PackageFilePartsUploadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PackageFilePartsUploadEntity)
  self._name = "package_file_parts_upload"
  self._client = client
...
function PackageFilePartsUploadEntity:get_name()
  return self._name
end


function PackageFilePartsUploadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PackageFilePartsUploadEntity.new(self._client, opts)
end
...
function PackageFilePartsUploadEntity:mark_deleted()
  self._deleted = true
end


function PackageFilePartsUploadEntity:deleted()
  return true == self._deleted
end


function PackageFilePartsUploadEntity:data_set(args)
  if args ~= nil then
...
function PackageFilePartsUploadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PackageFilePartsUploadEntity:match_set(args)
  if args ~= nil then
...
function PackageFilePartsUploadEntity:match_get()
...
function PackageFilePartsUploadEntity:stream(action, args, callopts)
...
---@param reqmatch PackageFilePartsUploadLoadMatch
---@param ctrl? table
---@return PackageFilePartsUpload
---@return string? err
function PackageFilePartsUploadEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PackageFilePartsUploadEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PackageFilePartsUploadEntity
