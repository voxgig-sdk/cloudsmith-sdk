-- Cloudsmith SDK Copy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CopyEntity = {}
CopyEntity.__index = CopyEntity


function CopyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CopyEntity)
  self._name = "copy"
  self._client = client
...
function CopyEntity:get_name()
  return self._name
end


function CopyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CopyEntity.new(self._client, opts)
end
...
function CopyEntity:mark_deleted()
  self._deleted = true
end


function CopyEntity:deleted()
  return true == self._deleted
end


function CopyEntity:data_set(args)
  if args ~= nil then
...
function CopyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CopyEntity:match_set(args)
  if args ~= nil then
...
function CopyEntity:stream(action, args, callopts)
...
function CopyEntity:_run_op(ctx, post_done)
  local utility = self._utility
