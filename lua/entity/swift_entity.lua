...
local SwiftEntity = {}
SwiftEntity.__index = SwiftEntity


function SwiftEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SwiftEntity)
  self._name = "swift"
  self._client = client
...
function SwiftEntity:get_name()
  return self._name
end


function SwiftEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SwiftEntity.new(self._client, opts)
end
...
function SwiftEntity:mark_deleted()
  self._deleted = true
end


function SwiftEntity:deleted()
  return true == self._deleted
end


function SwiftEntity:data_set(args)
  if args ~= nil then
...
function SwiftEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SwiftEntity:match_set(args)
  if args ~= nil then
...
function SwiftEntity:match_get()
...
function SwiftEntity:stream(action, args, callopts)
...
---@param reqmatch SwiftLoadMatch
---@param ctrl? table
---@return Swift
---@return string? err
function SwiftEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SwiftListMatch
---@param ctrl? table
---@return Swift[]
---@return string? err
function SwiftEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata SwiftCreateData
---@param ctrl? table
---@return Swift
---@return string? err
function SwiftEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata SwiftUpdateData
---@param ctrl? table
---@return Swift
---@return string? err
function SwiftEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function SwiftEntity:_run_op(ctx, post_done)
  local utility = self._utility
