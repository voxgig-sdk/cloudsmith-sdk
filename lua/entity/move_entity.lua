-- Cloudsmith SDK Move entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MoveEntity = {}
MoveEntity.__index = MoveEntity


function MoveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MoveEntity)
  self._name = "move"
  self._client = client
...
function MoveEntity:get_name()
  return self._name
end


function MoveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MoveEntity.new(self._client, opts)
end
...
function MoveEntity:mark_deleted()
  self._deleted = true
end


function MoveEntity:deleted()
  return true == self._deleted
end


function MoveEntity:data_set(args)
  if args ~= nil then
...
function MoveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MoveEntity:match_set(args)
  if args ~= nil then
...
function MoveEntity:stream(action, args, callopts)
...
function MoveEntity:_run_op(ctx, post_done)
  local utility = self._utility
