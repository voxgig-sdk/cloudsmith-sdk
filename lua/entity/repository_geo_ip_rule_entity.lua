-- Cloudsmith SDK RepositoryGeoIpRule entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RepositoryGeoIpRuleEntity = {}
RepositoryGeoIpRuleEntity.__index = RepositoryGeoIpRuleEntity


function RepositoryGeoIpRuleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepositoryGeoIpRuleEntity)
  self._name = "repository_geo_ip_rule"
  self._client = client
...
function RepositoryGeoIpRuleEntity:get_name()
  return self._name
end


function RepositoryGeoIpRuleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepositoryGeoIpRuleEntity.new(self._client, opts)
end
...
function RepositoryGeoIpRuleEntity:mark_deleted()
  self._deleted = true
end


function RepositoryGeoIpRuleEntity:deleted()
  return true == self._deleted
end


function RepositoryGeoIpRuleEntity:data_set(args)
  if args ~= nil then
...
function RepositoryGeoIpRuleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepositoryGeoIpRuleEntity:match_set(args)
  if args ~= nil then
...
function RepositoryGeoIpRuleEntity:match_get()
...
function RepositoryGeoIpRuleEntity:stream(action, args, callopts)
...
---@param reqmatch RepositoryGeoIpRuleLoadMatch
---@param ctrl? table
---@return RepositoryGeoIpRule
---@return string? err
function RepositoryGeoIpRuleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RepositoryGeoIpRuleUpdateData
---@param ctrl? table
---@return RepositoryGeoIpRule
---@return string? err
function RepositoryGeoIpRuleEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function RepositoryGeoIpRuleEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return RepositoryGeoIpRuleEntity
