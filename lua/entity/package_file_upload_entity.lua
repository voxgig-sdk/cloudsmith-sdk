-- Cloudsmith SDK PackageFileUpload entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PackageFileUploadEntity = {}
PackageFileUploadEntity.__index = PackageFileUploadEntity


function PackageFileUploadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PackageFileUploadEntity)
  self._name = "package_file_upload"
  self._client = client
...
function PackageFileUploadEntity:get_name()
  return self._name
end


function PackageFileUploadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PackageFileUploadEntity.new(self._client, opts)
end
...
function PackageFileUploadEntity:mark_deleted()
  self._deleted = true
end


function PackageFileUploadEntity:deleted()
  return true == self._deleted
end


function PackageFileUploadEntity:data_set(args)
  if args ~= nil then
...
function PackageFileUploadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PackageFileUploadEntity:match_set(args)
  if args ~= nil then
...
function PackageFileUploadEntity:match_get()
...
function PackageFileUploadEntity:stream(action, args, callopts)
...
---@param reqdata PackageFileUploadCreateData
---@param ctrl? table
---@return PackageFileUpload
---@return string? err
function PackageFileUploadEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PackageFileUploadEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PackageFileUploadEntity
