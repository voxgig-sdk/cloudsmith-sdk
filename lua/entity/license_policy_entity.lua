-- Cloudsmith SDK LicensePolicy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LicensePolicyEntity = {}
LicensePolicyEntity.__index = LicensePolicyEntity


function LicensePolicyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LicensePolicyEntity)
  self._name = "license_policy"
  self._client = client
...
function LicensePolicyEntity:get_name()
  return self._name
end


function LicensePolicyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LicensePolicyEntity.new(self._client, opts)
end
...
function LicensePolicyEntity:mark_deleted()
  self._deleted = true
end


function LicensePolicyEntity:deleted()
  return true == self._deleted
end


function LicensePolicyEntity:data_set(args)
  if args ~= nil then
...
function LicensePolicyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LicensePolicyEntity:match_set(args)
  if args ~= nil then
...
function LicensePolicyEntity:match_get()
...
function LicensePolicyEntity:stream(action, args, callopts)
...
function LicensePolicyEntity:_run_op(ctx, post_done)
  local utility = self._utility
