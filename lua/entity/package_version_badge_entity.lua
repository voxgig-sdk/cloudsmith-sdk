-- Cloudsmith SDK PackageVersionBadge entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PackageVersionBadgeEntity = {}
PackageVersionBadgeEntity.__index = PackageVersionBadgeEntity


function PackageVersionBadgeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PackageVersionBadgeEntity)
  self._name = "package_version_badge"
  self._client = client
...
function PackageVersionBadgeEntity:get_name()
  return self._name
end


function PackageVersionBadgeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PackageVersionBadgeEntity.new(self._client, opts)
end
...
function PackageVersionBadgeEntity:mark_deleted()
  self._deleted = true
end


function PackageVersionBadgeEntity:deleted()
  return true == self._deleted
end


function PackageVersionBadgeEntity:data_set(args)
  if args ~= nil then
...
function PackageVersionBadgeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PackageVersionBadgeEntity:match_set(args)
  if args ~= nil then
...
function PackageVersionBadgeEntity:match_get()
...
function PackageVersionBadgeEntity:stream(action, args, callopts)
...
---@param reqmatch PackageVersionBadgeLoadMatch
---@param ctrl? table
---@return PackageVersionBadge
---@return string? err
function PackageVersionBadgeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PackageVersionBadgeEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PackageVersionBadgeEntity
