-- Cloudsmith SDK Distro entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DistroEntity = {}
DistroEntity.__index = DistroEntity


function DistroEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DistroEntity)
  self._name = "distro"
  self._client = client
...
function DistroEntity:get_name()
  return self._name
end


function DistroEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DistroEntity.new(self._client, opts)
end
...
function DistroEntity:mark_deleted()
  self._deleted = true
end


function DistroEntity:deleted()
  return true == self._deleted
end


function DistroEntity:data_set(args)
  if args ~= nil then
...
function DistroEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DistroEntity:match_set(args)
  if args ~= nil then
...
function DistroEntity:match_get()
...
function DistroEntity:stream(action, args, callopts)
...
function DistroEntity:_run_op(ctx, post_done)
  local utility = self._utility
