-- Cloudsmith SDK Evaluation entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EvaluationEntity = {}
EvaluationEntity.__index = EvaluationEntity


function EvaluationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EvaluationEntity)
...
function EvaluationEntity:get_name()
  return self._name
end


function EvaluationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EvaluationEntity.new(self._client, opts)
end
...
function EvaluationEntity:mark_deleted()
  self._deleted = true
end


function EvaluationEntity:deleted()
  return true == self._deleted
end


function EvaluationEntity:data_set(args)
  if args ~= nil then
...
function EvaluationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EvaluationEntity:match_set(args)
  if args ~= nil then
...
function EvaluationEntity:match_get()
...
function EvaluationEntity:stream(action, args, callopts)
...
function EvaluationEntity:_run_op(ctx, post_done)
  local utility = self._utility
