-- Cloudsmith SDK Quarantine entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local QuarantineEntity = {}
QuarantineEntity.__index = QuarantineEntity


function QuarantineEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QuarantineEntity)
  self._name = "quarantine"
  self._client = client
...
function QuarantineEntity:get_name()
  return self._name
end


function QuarantineEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QuarantineEntity.new(self._client, opts)
end
...
function QuarantineEntity:mark_deleted()
  self._deleted = true
end


function QuarantineEntity:deleted()
  return true == self._deleted
end


function QuarantineEntity:data_set(args)
  if args ~= nil then
...
function QuarantineEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QuarantineEntity:match_set(args)
  if args ~= nil then
...
function QuarantineEntity:match_get()
...
function QuarantineEntity:stream(action, args, callopts)
...
function QuarantineEntity:_run_op(ctx, post_done)
  local utility = self._utility
