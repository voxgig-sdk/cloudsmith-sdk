-- Cloudsmith SDK Invite entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InviteEntity = {}
InviteEntity.__index = InviteEntity


function InviteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InviteEntity)
  self._name = "invite"
  self._client = client
...
function InviteEntity:get_name()
  return self._name
end


function InviteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InviteEntity.new(self._client, opts)
end
...
function InviteEntity:mark_deleted()
  self._deleted = true
end


function InviteEntity:deleted()
  return true == self._deleted
end


function InviteEntity:data_set(args)
  if args ~= nil then
...
function InviteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InviteEntity:match_set(args)
  if args ~= nil then
...
function InviteEntity:match_get()
...
function InviteEntity:stream(action, args, callopts)
...
function InviteEntity:_run_op(ctx, post_done)
  local utility = self._utility
