-- Cloudsmith SDK UserProfile entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local UserProfileEntity = {}
UserProfileEntity.__index = UserProfileEntity


function UserProfileEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserProfileEntity)
  self._name = "user_profile"
  self._client = client
...
function UserProfileEntity:get_name()
  return self._name
end


function UserProfileEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserProfileEntity.new(self._client, opts)
end
...
function UserProfileEntity:mark_deleted()
  self._deleted = true
end


function UserProfileEntity:deleted()
  return true == self._deleted
end


function UserProfileEntity:data_set(args)
  if args ~= nil then
...
function UserProfileEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserProfileEntity:match_set(args)
  if args ~= nil then
...
function UserProfileEntity:match_get()
...
function UserProfileEntity:stream(action, args, callopts)
...
---@param reqmatch UserProfileLoadMatch
---@param ctrl? table
---@return UserProfile
---@return string? err
function UserProfileEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function UserProfileEntity:_run_op(ctx, post_done)
  local utility = self._utility
