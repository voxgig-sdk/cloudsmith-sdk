-- Cloudsmith SDK Disable entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DisableEntity = {}
DisableEntity.__index = DisableEntity


function DisableEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DisableEntity)
  self._name = "disable"
  self._client = client
...
function DisableEntity:get_name()
  return self._name
end


function DisableEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DisableEntity.new(self._client, opts)
end
...
function DisableEntity:mark_deleted()
  self._deleted = true
end


function DisableEntity:deleted()
  return true == self._deleted
end


function DisableEntity:data_set(args)
  if args ~= nil then
...
function DisableEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DisableEntity:match_set(args)
  if args ~= nil then
...
function DisableEntity:match_get()
...
function DisableEntity:stream(action, args, callopts)
...
function DisableEntity:_run_op(ctx, post_done)
  local utility = self._utility
