...
local FormatEntity = {}
FormatEntity.__index = FormatEntity


function FormatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FormatEntity)
  self._name = "format"
  self._client = client
...
function FormatEntity:get_name()
  return self._name
end


function FormatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FormatEntity.new(self._client, opts)
end
...
function FormatEntity:mark_deleted()
  self._deleted = true
end


function FormatEntity:deleted()
  return true == self._deleted
end


function FormatEntity:data_set(args)
  if args ~= nil then
...
function FormatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FormatEntity:match_set(args)
  if args ~= nil then
...
function FormatEntity:match_get()
...
function FormatEntity:stream(action, args, callopts)
...
---@param reqmatch FormatLoadMatch
---@param ctrl? table
---@return Format
---@return string? err
function FormatEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FormatEntity:_run_op(ctx, post_done)
  local utility = self._utility
