-- Cloudsmith SDK RepositoryPrivilegeInput entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RepositoryPrivilegeInputEntity = {}
RepositoryPrivilegeInputEntity.__index = RepositoryPrivilegeInputEntity


function RepositoryPrivilegeInputEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepositoryPrivilegeInputEntity)
  self._name = "repository_privilege_input"
  self._client = client
...
function RepositoryPrivilegeInputEntity:get_name()
  return self._name
end


function RepositoryPrivilegeInputEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepositoryPrivilegeInputEntity.new(self._client, opts)
end
...
function RepositoryPrivilegeInputEntity:mark_deleted()
  self._deleted = true
end


function RepositoryPrivilegeInputEntity:deleted()
  return true == self._deleted
end


function RepositoryPrivilegeInputEntity:data_set(args)
  if args ~= nil then
...
function RepositoryPrivilegeInputEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepositoryPrivilegeInputEntity:match_set(args)
  if args ~= nil then
...
function RepositoryPrivilegeInputEntity:match_get()
...
function RepositoryPrivilegeInputEntity:stream(action, args, callopts)
...
---@param reqmatch RepositoryPrivilegeInputListMatch
---@param ctrl? table
---@return RepositoryPrivilegeInput[]
---@return string? err
function RepositoryPrivilegeInputEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RepositoryPrivilegeInputEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return RepositoryPrivilegeInputEntity
