-- Cloudsmith SDK Rsa entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RsaEntity = {}
RsaEntity.__index = RsaEntity


function RsaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RsaEntity)
  self._name = "rsa"
  self._client = client
...
function RsaEntity:get_name()
  return self._name
end


function RsaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RsaEntity.new(self._client, opts)
end
...
function RsaEntity:mark_deleted()
  self._deleted = true
end


function RsaEntity:deleted()
  return true == self._deleted
end


function RsaEntity:data_set(args)
  if args ~= nil then
...
function RsaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RsaEntity:match_set(args)
  if args ~= nil then
...
function RsaEntity:stream(action, args, callopts)
...
function RsaEntity:_run_op(ctx, post_done)
  local utility = self._utility
