-- Cloudsmith SDK Self entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SelfEntity = {}
SelfEntity.__index = SelfEntity


function SelfEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SelfEntity)
  self._name = "self"
  self._client = client
...
function SelfEntity:get_name()
  return self._name
end


function SelfEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SelfEntity.new(self._client, opts)
end
...
function SelfEntity:mark_deleted()
  self._deleted = true
end


function SelfEntity:deleted()
  return true == self._deleted
end


function SelfEntity:data_set(args)
  if args ~= nil then
...
function SelfEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SelfEntity:match_set(args)
  if args ~= nil then
...
function SelfEntity:stream(action, args, callopts)
...
function SelfEntity:_run_op(ctx, post_done)
  local utility = self._utility
