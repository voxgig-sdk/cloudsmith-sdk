...
local DynamicMappingEntity = {}
DynamicMappingEntity.__index = DynamicMappingEntity


function DynamicMappingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DynamicMappingEntity)
  self._name = "dynamic_mapping"
  self._client = client
...
function DynamicMappingEntity:get_name()
  return self._name
end


function DynamicMappingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DynamicMappingEntity.new(self._client, opts)
end
...
function DynamicMappingEntity:mark_deleted()
  self._deleted = true
end


function DynamicMappingEntity:deleted()
  return true == self._deleted
end


function DynamicMappingEntity:data_set(args)
  if args ~= nil then
...
function DynamicMappingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DynamicMappingEntity:match_set(args)
  if args ~= nil then
...
function DynamicMappingEntity:match_get()
...
function DynamicMappingEntity:stream(action, args, callopts)
...
---@param reqmatch DynamicMappingLoadMatch
---@param ctrl? table
---@return DynamicMapping
---@return string? err
function DynamicMappingEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DynamicMappingListMatch
---@param ctrl? table
---@return DynamicMapping[]
---@return string? err
function DynamicMappingEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function DynamicMappingEntity:_run_op(ctx, post_done)
  local utility = self._utility
