-- Cloudsmith SDK Validate entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ValidateEntity = {}
ValidateEntity.__index = ValidateEntity


function ValidateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ValidateEntity)
  self._name = "validate"
  self._client = client
...
function ValidateEntity:get_name()
  return self._name
end


function ValidateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ValidateEntity.new(self._client, opts)
end
...
function ValidateEntity:mark_deleted()
  self._deleted = true
end


function ValidateEntity:deleted()
  return true == self._deleted
end


function ValidateEntity:data_set(args)
  if args ~= nil then
...
function ValidateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ValidateEntity:match_set(args)
  if args ~= nil then
...
function ValidateEntity:match_get()
...
function ValidateEntity:stream(action, args, callopts)
...
function ValidateEntity:_run_op(ctx, post_done)
  local utility = self._utility
