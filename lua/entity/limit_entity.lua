-- Cloudsmith SDK Limit entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LimitEntity = {}
LimitEntity.__index = LimitEntity


function LimitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LimitEntity)
  self._name = "limit"
  self._client = client
...
function LimitEntity:get_name()
  return self._name
end


function LimitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LimitEntity.new(self._client, opts)
end
...
function LimitEntity:mark_deleted()
  self._deleted = true
end


function LimitEntity:deleted()
  return true == self._deleted
end


function LimitEntity:data_set(args)
  if args ~= nil then
...
function LimitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LimitEntity:match_set(args)
  if args ~= nil then
...
function LimitEntity:match_get()
...
function LimitEntity:stream(action, args, callopts)
...
function LimitEntity:_run_op(ctx, post_done)
  local utility = self._utility
