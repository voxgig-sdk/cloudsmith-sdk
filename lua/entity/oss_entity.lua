-- Cloudsmith SDK Oss entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OssEntity = {}
OssEntity.__index = OssEntity


function OssEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OssEntity)
  self._name = "oss"
  self._client = client
...
function OssEntity:get_name()
  return self._name
end


function OssEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OssEntity.new(self._client, opts)
end
...
function OssEntity:mark_deleted()
  self._deleted = true
end


function OssEntity:deleted()
  return true == self._deleted
end


function OssEntity:data_set(args)
  if args ~= nil then
...
function OssEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OssEntity:match_set(args)
  if args ~= nil then
...
function OssEntity:stream(action, args, callopts)
...
function OssEntity:_run_op(ctx, post_done)
  local utility = self._utility
