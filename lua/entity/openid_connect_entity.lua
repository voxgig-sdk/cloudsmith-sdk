-- Cloudsmith SDK OpenidConnect entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OpenidConnectEntity = {}
OpenidConnectEntity.__index = OpenidConnectEntity


function OpenidConnectEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OpenidConnectEntity)
  self._name = "openid_connect"
  self._client = client
...
function OpenidConnectEntity:get_name()
  return self._name
end


function OpenidConnectEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OpenidConnectEntity.new(self._client, opts)
end
...
function OpenidConnectEntity:mark_deleted()
  self._deleted = true
end


function OpenidConnectEntity:deleted()
  return true == self._deleted
end


function OpenidConnectEntity:data_set(args)
  if args ~= nil then
...
function OpenidConnectEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OpenidConnectEntity:match_set(args)
  if args ~= nil then
...
function OpenidConnectEntity:match_get()
...
function OpenidConnectEntity:stream(action, args, callopts)
...
function OpenidConnectEntity:_run_op(ctx, post_done)
  local utility = self._utility
