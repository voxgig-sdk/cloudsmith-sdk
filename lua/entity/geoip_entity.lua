-- Cloudsmith SDK Geoip entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GeoipEntity = {}
GeoipEntity.__index = GeoipEntity


function GeoipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeoipEntity)
  self._name = "geoip"
  self._client = client
...
function GeoipEntity:get_name()
  return self._name
end


function GeoipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeoipEntity.new(self._client, opts)
end
...
function GeoipEntity:mark_deleted()
  self._deleted = true
end


function GeoipEntity:deleted()
  return true == self._deleted
end


function GeoipEntity:data_set(args)
  if args ~= nil then
...
function GeoipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeoipEntity:match_set(args)
  if args ~= nil then
...
function GeoipEntity:match_get()
...
function GeoipEntity:stream(action, args, callopts)
...
function GeoipEntity:_run_op(ctx, post_done)
  local utility = self._utility
