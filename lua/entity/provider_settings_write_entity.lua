-- Cloudsmith SDK ProviderSettingsWrite entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ProviderSettingsWriteEntity = {}
ProviderSettingsWriteEntity.__index = ProviderSettingsWriteEntity


function ProviderSettingsWriteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProviderSettingsWriteEntity)
  self._name = "provider_settings_write"
  self._client = client
...
function ProviderSettingsWriteEntity:get_name()
  return self._name
end


function ProviderSettingsWriteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProviderSettingsWriteEntity.new(self._client, opts)
end
...
function ProviderSettingsWriteEntity:mark_deleted()
  self._deleted = true
end


function ProviderSettingsWriteEntity:deleted()
  return true == self._deleted
end


function ProviderSettingsWriteEntity:data_set(args)
  if args ~= nil then
...
function ProviderSettingsWriteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProviderSettingsWriteEntity:match_set(args)
  if args ~= nil then
...
function ProviderSettingsWriteEntity:match_get()
...
function ProviderSettingsWriteEntity:stream(action, args, callopts)
...
---@param reqdata ProviderSettingsWriteCreateData
---@param ctrl? table
---@return ProviderSettingsWrite
---@return string? err
function ProviderSettingsWriteEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqdata ProviderSettingsWriteUpdateData
---@param ctrl? table
---@return ProviderSettingsWrite
---@return string? err
function ProviderSettingsWriteEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function ProviderSettingsWriteEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ProviderSettingsWriteEntity
