-- Cloudsmith SDK Retention entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RetentionEntity = {}
RetentionEntity.__index = RetentionEntity


function RetentionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RetentionEntity)
  self._name = "retention"
  self._client = client
...
function RetentionEntity:get_name()
  return self._name
end


function RetentionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RetentionEntity.new(self._client, opts)
end
...
function RetentionEntity:mark_deleted()
  self._deleted = true
end


function RetentionEntity:deleted()
  return true == self._deleted
end


function RetentionEntity:data_set(args)
  if args ~= nil then
...
function RetentionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RetentionEntity:match_set(args)
  if args ~= nil then
...
function RetentionEntity:match_get()
...
function RetentionEntity:stream(action, args, callopts)
...
function RetentionEntity:_run_op(ctx, post_done)
  local utility = self._utility
