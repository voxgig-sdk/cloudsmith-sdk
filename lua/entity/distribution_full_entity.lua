...
local DistributionFullEntity = {}
DistributionFullEntity.__index = DistributionFullEntity


function DistributionFullEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DistributionFullEntity)
  self._name = "distribution_full"
  self._client = client
...
function DistributionFullEntity:get_name()
  return self._name
end


function DistributionFullEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DistributionFullEntity.new(self._client, opts)
end
...
function DistributionFullEntity:mark_deleted()
  self._deleted = true
end


function DistributionFullEntity:deleted()
  return true == self._deleted
end


function DistributionFullEntity:data_set(args)
  if args ~= nil then
...
function DistributionFullEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DistributionFullEntity:match_set(args)
  if args ~= nil then
...
function DistributionFullEntity:match_get()
...
function DistributionFullEntity:stream(action, args, callopts)
...
---@param reqmatch DistributionFullLoadMatch
---@param ctrl? table
---@return DistributionFull
---@return string? err
function DistributionFullEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DistributionFullListMatch
---@param ctrl? table
---@return DistributionFull[]
---@return string? err
function DistributionFullEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function DistributionFullEntity:_run_op(ctx, post_done)
  local utility = self._utility
