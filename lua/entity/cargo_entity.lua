...
local CargoEntity = {}
CargoEntity.__index = CargoEntity


function CargoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CargoEntity)
  self._name = "cargo"
  self._client = client
...
function CargoEntity:get_name()
  return self._name
end


function CargoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CargoEntity.new(self._client, opts)
end
...
function CargoEntity:mark_deleted()
  self._deleted = true
end


function CargoEntity:deleted()
  return true == self._deleted
end


function CargoEntity:data_set(args)
  if args ~= nil then
...
function CargoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CargoEntity:match_set(args)
  if args ~= nil then
...
function CargoEntity:match_get()
...
function CargoEntity:stream(action, args, callopts)
...
---@param reqmatch CargoLoadMatch
---@param ctrl? table
---@return Cargo
---@return string? err
function CargoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CargoListMatch
---@param ctrl? table
---@return Cargo[]
---@return string? err
function CargoEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CargoCreateData
---@param ctrl? table
---@return Cargo
---@return string? err
function CargoEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata CargoUpdateData
---@param ctrl? table
---@return Cargo
---@return string? err
function CargoEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function CargoEntity:_run_op(ctx, post_done)
  local utility = self._utility
