-- Cloudsmith SDK DenyPolicy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DenyPolicyEntity = {}
DenyPolicyEntity.__index = DenyPolicyEntity


function DenyPolicyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DenyPolicyEntity)
  self._name = "deny_policy"
  self._client = client
...
function DenyPolicyEntity:get_name()
  return self._name
end


function DenyPolicyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DenyPolicyEntity.new(self._client, opts)
end
...
function DenyPolicyEntity:mark_deleted()
  self._deleted = true
end


function DenyPolicyEntity:deleted()
  return true == self._deleted
end


function DenyPolicyEntity:data_set(args)
  if args ~= nil then
...
function DenyPolicyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DenyPolicyEntity:match_set(args)
  if args ~= nil then
...
function DenyPolicyEntity:match_get()
...
function DenyPolicyEntity:stream(action, args, callopts)
...
function DenyPolicyEntity:_run_op(ctx, post_done)
  local utility = self._utility
