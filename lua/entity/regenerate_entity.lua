-- Cloudsmith SDK Regenerate entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RegenerateEntity = {}
RegenerateEntity.__index = RegenerateEntity


function RegenerateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RegenerateEntity)
  self._name = "regenerate"
  self._client = client
...
function RegenerateEntity:get_name()
  return self._name
end


function RegenerateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RegenerateEntity.new(self._client, opts)
end
...
function RegenerateEntity:mark_deleted()
  self._deleted = true
end


function RegenerateEntity:deleted()
  return true == self._deleted
end


function RegenerateEntity:data_set(args)
  if args ~= nil then
...
function RegenerateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RegenerateEntity:match_set(args)
  if args ~= nil then
...
function RegenerateEntity:match_get()
...
function RegenerateEntity:stream(action, args, callopts)
...
function RegenerateEntity:_run_op(ctx, post_done)
  local utility = self._utility
