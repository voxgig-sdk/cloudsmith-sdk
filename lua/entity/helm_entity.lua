...
local HelmEntity = {}
HelmEntity.__index = HelmEntity


function HelmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HelmEntity)
  self._name = "helm"
  self._client = client
...
function HelmEntity:get_name()
  return self._name
end


function HelmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HelmEntity.new(self._client, opts)
end
...
function HelmEntity:mark_deleted()
  self._deleted = true
end


function HelmEntity:deleted()
  return true == self._deleted
end


function HelmEntity:data_set(args)
  if args ~= nil then
...
function HelmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HelmEntity:match_set(args)
  if args ~= nil then
...
function HelmEntity:stream(action, args, callopts)
...
---@param reqmatch HelmLoadMatch
---@param ctrl? table
---@return Helm
---@return string? err
function HelmEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch HelmListMatch
---@param ctrl? table
---@return Helm[]
---@return string? err
function HelmEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata HelmCreateData
---@param ctrl? table
---@return Helm
---@return string? err
function HelmEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata HelmUpdateData
---@param ctrl? table
---@return Helm
---@return string? err
function HelmEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function HelmEntity:_run_op(ctx, post_done)
  local utility = self._utility
