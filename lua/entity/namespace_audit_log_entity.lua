-- Cloudsmith SDK NamespaceAuditLog entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local NamespaceAuditLogEntity = {}
NamespaceAuditLogEntity.__index = NamespaceAuditLogEntity


function NamespaceAuditLogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NamespaceAuditLogEntity)
  self._name = "namespace_audit_log"
  self._client = client
...
function NamespaceAuditLogEntity:get_name()
  return self._name
end


function NamespaceAuditLogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NamespaceAuditLogEntity.new(self._client, opts)
end
...
function NamespaceAuditLogEntity:mark_deleted()
  self._deleted = true
end


function NamespaceAuditLogEntity:deleted()
  return true == self._deleted
end


function NamespaceAuditLogEntity:data_set(args)
  if args ~= nil then
...
function NamespaceAuditLogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NamespaceAuditLogEntity:match_set(args)
  if args ~= nil then
...
function NamespaceAuditLogEntity:match_get()
...
function NamespaceAuditLogEntity:stream(action, args, callopts)
...
---@param reqmatch NamespaceAuditLogLoadMatch
---@param ctrl? table
---@return NamespaceAuditLog
---@return string? err
function NamespaceAuditLogEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NamespaceAuditLogEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return NamespaceAuditLogEntity
