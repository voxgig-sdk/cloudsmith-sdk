-- Cloudsmith SDK UserAuthToken entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local UserAuthTokenEntity = {}
UserAuthTokenEntity.__index = UserAuthTokenEntity


function UserAuthTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserAuthTokenEntity)
  self._name = "user_auth_token"
  self._client = client
...
function UserAuthTokenEntity:get_name()
  return self._name
end


function UserAuthTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserAuthTokenEntity.new(self._client, opts)
end
...
function UserAuthTokenEntity:mark_deleted()
  self._deleted = true
end


function UserAuthTokenEntity:deleted()
  return true == self._deleted
end


function UserAuthTokenEntity:data_set(args)
  if args ~= nil then
...
function UserAuthTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserAuthTokenEntity:match_set(args)
  if args ~= nil then
...
function UserAuthTokenEntity:match_get()
...
function UserAuthTokenEntity:stream(action, args, callopts)
...
---@param reqdata UserAuthTokenCreateData
---@param ctrl? table
---@return UserAuthToken
---@return string? err
function UserAuthTokenEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function UserAuthTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
