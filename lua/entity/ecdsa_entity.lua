-- Cloudsmith SDK Ecdsa entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EcdsaEntity = {}
EcdsaEntity.__index = EcdsaEntity


function EcdsaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EcdsaEntity)
  self._name = "ecdsa"
  self._client = client
...
function EcdsaEntity:get_name()
  return self._name
end


function EcdsaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EcdsaEntity.new(self._client, opts)
end
...
function EcdsaEntity:mark_deleted()
  self._deleted = true
end


function EcdsaEntity:deleted()
  return true == self._deleted
end


function EcdsaEntity:data_set(args)
  if args ~= nil then
...
function EcdsaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EcdsaEntity:match_set(args)
  if args ~= nil then
...
function EcdsaEntity:match_get()
...
function EcdsaEntity:stream(action, args, callopts)
...
function EcdsaEntity:_run_op(ctx, post_done)
  local utility = self._utility
