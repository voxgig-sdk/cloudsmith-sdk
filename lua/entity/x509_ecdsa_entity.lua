-- Cloudsmith SDK X509Ecdsa entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local X509EcdsaEntity = {}
X509EcdsaEntity.__index = X509EcdsaEntity


function X509EcdsaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, X509EcdsaEntity)
  self._name = "x509_ecdsa"
  self._client = client
...
function X509EcdsaEntity:get_name()
  return self._name
end


function X509EcdsaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return X509EcdsaEntity.new(self._client, opts)
end
...
function X509EcdsaEntity:mark_deleted()
  self._deleted = true
end


function X509EcdsaEntity:deleted()
  return true == self._deleted
end


function X509EcdsaEntity:data_set(args)
  if args ~= nil then
...
function X509EcdsaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function X509EcdsaEntity:match_set(args)
  if args ~= nil then
...
function X509EcdsaEntity:match_get()
...
function X509EcdsaEntity:stream(action, args, callopts)
...
function X509EcdsaEntity:_run_op(ctx, post_done)
  local utility = self._utility
