...
local StorageRegionEntity = {}
StorageRegionEntity.__index = StorageRegionEntity


function StorageRegionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StorageRegionEntity)
  self._name = "storage_region"
  self._client = client
...
function StorageRegionEntity:get_name()
  return self._name
end


function StorageRegionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StorageRegionEntity.new(self._client, opts)
end
...
function StorageRegionEntity:mark_deleted()
  self._deleted = true
end


function StorageRegionEntity:deleted()
  return true == self._deleted
end


function StorageRegionEntity:data_set(args)
  if args ~= nil then
...
function StorageRegionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StorageRegionEntity:match_set(args)
  if args ~= nil then
...
function StorageRegionEntity:match_get()
...
function StorageRegionEntity:stream(action, args, callopts)
...
---@param reqmatch StorageRegionLoadMatch
---@param ctrl? table
---@return StorageRegion
---@return string? err
function StorageRegionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch StorageRegionListMatch
---@param ctrl? table
---@return StorageRegion[]
---@return string? err
function StorageRegionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function StorageRegionEntity:_run_op(ctx, post_done)
  local utility = self._utility
