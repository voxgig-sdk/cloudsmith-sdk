-- Cloudsmith SDK P2n entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local P2nEntity = {}
P2nEntity.__index = P2nEntity


function P2nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, P2nEntity)
  self._name = "p2n"
  self._client = client
...
function P2nEntity:get_name()
  return self._name
end


function P2nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return P2nEntity.new(self._client, opts)
end
...
function P2nEntity:mark_deleted()
  self._deleted = true
end


function P2nEntity:deleted()
  return true == self._deleted
end


function P2nEntity:data_set(args)
  if args ~= nil then
...
function P2nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function P2nEntity:match_set(args)
  if args ~= nil then
...
function P2nEntity:stream(action, args, callopts)
...
function P2nEntity:_run_op(ctx, post_done)
  local utility = self._utility
