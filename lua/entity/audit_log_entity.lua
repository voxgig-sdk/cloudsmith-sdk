-- Cloudsmith SDK AuditLog entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AuditLogEntity = {}
AuditLogEntity.__index = AuditLogEntity


function AuditLogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuditLogEntity)
  self._name = "audit_log"
  self._client = client
...
function AuditLogEntity:get_name()
  return self._name
end


function AuditLogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuditLogEntity.new(self._client, opts)
end
...
function AuditLogEntity:mark_deleted()
  self._deleted = true
end


function AuditLogEntity:deleted()
  return true == self._deleted
end


function AuditLogEntity:data_set(args)
  if args ~= nil then
...
function AuditLogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuditLogEntity:match_set(args)
  if args ~= nil then
...
function AuditLogEntity:match_get()
...
function AuditLogEntity:stream(action, args, callopts)
...
function AuditLogEntity:_run_op(ctx, post_done)
  local utility = self._utility
