...
local EntitlementEntity = {}
EntitlementEntity.__index = EntitlementEntity


function EntitlementEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EntitlementEntity)
  self._name = "entitlement"
  self._client = client
...
function EntitlementEntity:get_name()
  return self._name
end


function EntitlementEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EntitlementEntity.new(self._client, opts)
end
...
function EntitlementEntity:mark_deleted()
  self._deleted = true
end


function EntitlementEntity:deleted()
  return true == self._deleted
end


function EntitlementEntity:data_set(args)
  if args ~= nil then
...
function EntitlementEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EntitlementEntity:match_set(args)
  if args ~= nil then
...
function EntitlementEntity:match_get()
...
function EntitlementEntity:stream(action, args, callopts)
...
---@param reqmatch EntitlementLoadMatch
---@param ctrl? table
---@return Entitlement
---@return string? err
function EntitlementEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata EntitlementCreateData
---@param ctrl? table
---@return Entitlement
---@return string? err
function EntitlementEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch EntitlementRemoveMatch
---@param ctrl? table
---@return Entitlement
---@return string? err
function EntitlementEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EntitlementEntity:_run_op(ctx, post_done)
  local utility = self._utility
