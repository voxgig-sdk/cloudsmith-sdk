-- Cloudsmith SDK Scan entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ScanEntity = {}
ScanEntity.__index = ScanEntity


function ScanEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ScanEntity)
  self._name = "scan"
  self._client = client
...
function ScanEntity:get_name()
  return self._name
end


function ScanEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ScanEntity.new(self._client, opts)
end
...
function ScanEntity:mark_deleted()
  self._deleted = true
end


function ScanEntity:deleted()
  return true == self._deleted
end


function ScanEntity:data_set(args)
  if args ~= nil then
...
function ScanEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ScanEntity:match_set(args)
  if args ~= nil then
...
function ScanEntity:stream(action, args, callopts)
...
function ScanEntity:_run_op(ctx, post_done)
  local utility = self._utility
