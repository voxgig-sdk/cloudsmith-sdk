-- Cloudsmith SDK Dependency entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DependencyEntity = {}
DependencyEntity.__index = DependencyEntity


function DependencyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DependencyEntity)
...
function DependencyEntity:get_name()
  return self._name
end


function DependencyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DependencyEntity.new(self._client, opts)
end
...
function DependencyEntity:mark_deleted()
  self._deleted = true
end


function DependencyEntity:deleted()
  return true == self._deleted
end


function DependencyEntity:data_set(args)
  if args ~= nil then
...
function DependencyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DependencyEntity:match_set(args)
  if args ~= nil then
...
function DependencyEntity:match_get()
...
function DependencyEntity:stream(action, args, callopts)
...
function DependencyEntity:_run_op(ctx, post_done)
  local utility = self._utility
