-- Cloudsmith SDK RepositoryTokenSync entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RepositoryTokenSyncEntity = {}
RepositoryTokenSyncEntity.__index = RepositoryTokenSyncEntity


function RepositoryTokenSyncEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepositoryTokenSyncEntity)
  self._name = "repository_token_sync"
  self._client = client
...
function RepositoryTokenSyncEntity:get_name()
  return self._name
end


function RepositoryTokenSyncEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepositoryTokenSyncEntity.new(self._client, opts)
end
...
function RepositoryTokenSyncEntity:mark_deleted()
  self._deleted = true
end


function RepositoryTokenSyncEntity:deleted()
  return true == self._deleted
end


function RepositoryTokenSyncEntity:data_set(args)
  if args ~= nil then
...
function RepositoryTokenSyncEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepositoryTokenSyncEntity:match_set(args)
  if args ~= nil then
...
function RepositoryTokenSyncEntity:match_get()
...
function RepositoryTokenSyncEntity:stream(action, args, callopts)
...
---@param reqdata RepositoryTokenSyncCreateData
---@param ctrl? table
---@return RepositoryTokenSync
---@return string? err
function RepositoryTokenSyncEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RepositoryTokenSyncEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return RepositoryTokenSyncEntity
