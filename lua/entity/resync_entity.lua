-- Cloudsmith SDK Resync entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ResyncEntity = {}
ResyncEntity.__index = ResyncEntity


function ResyncEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResyncEntity)
  self._name = "resync"
  self._client = client
...
function ResyncEntity:get_name()
  return self._name
end


function ResyncEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResyncEntity.new(self._client, opts)
end
...
function ResyncEntity:mark_deleted()
  self._deleted = true
end


function ResyncEntity:deleted()
  return true == self._deleted
end


function ResyncEntity:data_set(args)
  if args ~= nil then
...
function ResyncEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResyncEntity:match_set(args)
  if args ~= nil then
...
function ResyncEntity:match_get()
...
function ResyncEntity:stream(action, args, callopts)
...
function ResyncEntity:_run_op(ctx, post_done)
  local utility = self._utility
