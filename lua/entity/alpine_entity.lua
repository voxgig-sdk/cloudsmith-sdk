-- Cloudsmith SDK Alpine entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AlpineEntity = {}
AlpineEntity.__index = AlpineEntity


function AlpineEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AlpineEntity)
  self._name = "alpine"
  self._client = client
...
function AlpineEntity:get_name()
  return self._name
end


function AlpineEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AlpineEntity.new(self._client, opts)
end
...
function AlpineEntity:mark_deleted()
  self._deleted = true
end


function AlpineEntity:deleted()
  return true == self._deleted
end


function AlpineEntity:data_set(args)
  if args ~= nil then
...
function AlpineEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AlpineEntity:match_set(args)
  if args ~= nil then
...
function AlpineEntity:match_get()
...
function AlpineEntity:stream(action, args, callopts)
...
function AlpineEntity:_run_op(ctx, post_done)
  local utility = self._utility
