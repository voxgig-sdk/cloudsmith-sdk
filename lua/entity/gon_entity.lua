-- Cloudsmith SDK Gon entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GonEntity = {}
GonEntity.__index = GonEntity


function GonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GonEntity)
  self._name = "gon"
  self._client = client
...
function GonEntity:get_name()
  return self._name
end


function GonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GonEntity.new(self._client, opts)
end
...
function GonEntity:mark_deleted()
  self._deleted = true
end


function GonEntity:deleted()
  return true == self._deleted
end


function GonEntity:data_set(args)
  if args ~= nil then
...
function GonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GonEntity:match_set(args)
  if args ~= nil then
...
function GonEntity:stream(action, args, callopts)
...
---@param reqmatch GonLoadMatch
---@param ctrl? table
---@return Gon
---@return string? err
function GonEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GonListMatch
---@param ctrl? table
---@return Gon[]
---@return string? err
function GonEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata GonCreateData
---@param ctrl? table
---@return Gon
---@return string? err
function GonEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata GonUpdateData
---@param ctrl? table
---@return Gon
---@return string? err
function GonEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function GonEntity:_run_op(ctx, post_done)
  local utility = self._utility
