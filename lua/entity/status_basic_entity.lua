-- Cloudsmith SDK StatusBasic entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local StatusBasicEntity = {}
StatusBasicEntity.__index = StatusBasicEntity


function StatusBasicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StatusBasicEntity)
  self._name = "status_basic"
  self._client = client
...
function StatusBasicEntity:get_name()
  return self._name
end


function StatusBasicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StatusBasicEntity.new(self._client, opts)
end
...
function StatusBasicEntity:mark_deleted()
  self._deleted = true
end


function StatusBasicEntity:deleted()
  return true == self._deleted
end


function StatusBasicEntity:data_set(args)
  if args ~= nil then
...
function StatusBasicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StatusBasicEntity:match_set(args)
  if args ~= nil then
...
function StatusBasicEntity:match_get()
...
function StatusBasicEntity:stream(action, args, callopts)
...
---@param reqmatch StatusBasicLoadMatch
---@param ctrl? table
---@return StatusBasic
---@return string? err
function StatusBasicEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StatusBasicEntity:_run_op(ctx, post_done)
  local utility = self._utility
