...
local MavenEntity = {}
MavenEntity.__index = MavenEntity


function MavenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MavenEntity)
  self._name = "maven"
  self._client = client
...
function MavenEntity:get_name()
  return self._name
end


function MavenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MavenEntity.new(self._client, opts)
end
...
function MavenEntity:mark_deleted()
  self._deleted = true
end


function MavenEntity:deleted()
  return true == self._deleted
end


function MavenEntity:data_set(args)
  if args ~= nil then
...
function MavenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MavenEntity:match_set(args)
  if args ~= nil then
...
function MavenEntity:match_get()
...
function MavenEntity:stream(action, args, callopts)
...
---@param reqmatch MavenLoadMatch
---@param ctrl? table
---@return Maven
---@return string? err
function MavenEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch MavenListMatch
---@param ctrl? table
---@return Maven[]
---@return string? err
function MavenEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata MavenCreateData
---@param ctrl? table
---@return Maven
---@return string? err
function MavenEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata MavenUpdateData
---@param ctrl? table
---@return Maven
---@return string? err
function MavenEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function MavenEntity:_run_op(ctx, post_done)
  local utility = self._utility
