...
local DockerEntity = {}
DockerEntity.__index = DockerEntity


function DockerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DockerEntity)
  self._name = "docker"
  self._client = client
...
function DockerEntity:get_name()
  return self._name
end


function DockerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DockerEntity.new(self._client, opts)
end
...
function DockerEntity:mark_deleted()
  self._deleted = true
end


function DockerEntity:deleted()
  return true == self._deleted
end


function DockerEntity:data_set(args)
  if args ~= nil then
...
function DockerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DockerEntity:match_set(args)
  if args ~= nil then
...
function DockerEntity:match_get()
...
function DockerEntity:stream(action, args, callopts)
...
---@param reqmatch DockerLoadMatch
---@param ctrl? table
---@return Docker
---@return string? err
function DockerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DockerListMatch
---@param ctrl? table
---@return Docker[]
---@return string? err
function DockerEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata DockerCreateData
---@param ctrl? table
---@return Docker
---@return string? err
function DockerEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata DockerUpdateData
---@param ctrl? table
---@return Docker
---@return string? err
function DockerEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function DockerEntity:_run_op(ctx, post_done)
  local utility = self._utility
