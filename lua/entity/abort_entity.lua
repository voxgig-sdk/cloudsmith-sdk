-- Cloudsmith SDK Abort entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AbortEntity = {}
AbortEntity.__index = AbortEntity


function AbortEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AbortEntity)
  self._name = "abort"
  self._client = client
...
function AbortEntity:get_name()
  return self._name
end


function AbortEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AbortEntity.new(self._client, opts)
end
...
function AbortEntity:mark_deleted()
  self._deleted = true
end


function AbortEntity:deleted()
  return true == self._deleted
end


function AbortEntity:data_set(args)
  if args ~= nil then
...
function AbortEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AbortEntity:match_set(args)
  if args ~= nil then
...
function AbortEntity:match_get()
...
function AbortEntity:stream(action, args, callopts)
...
function AbortEntity:_run_op(ctx, post_done)
  local utility = self._utility
