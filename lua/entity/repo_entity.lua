...
local RepoEntity = {}
RepoEntity.__index = RepoEntity


function RepoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepoEntity)
  self._name = "repo"
  self._client = client
...
function RepoEntity:get_name()
  return self._name
end


function RepoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepoEntity.new(self._client, opts)
end
...
function RepoEntity:mark_deleted()
  self._deleted = true
end


function RepoEntity:deleted()
  return true == self._deleted
end


function RepoEntity:data_set(args)
  if args ~= nil then
...
function RepoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepoEntity:match_set(args)
  if args ~= nil then
...
function RepoEntity:stream(action, args, callopts)
...
---@param reqdata RepoCreateData
---@param ctrl? table
---@return Repo
---@return string? err
function RepoEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata RepoUpdateData
---@param ctrl? table
---@return Repo
---@return string? err
function RepoEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch RepoRemoveMatch
---@param ctrl? table
---@return Repo
---@return string? err
function RepoEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function RepoEntity:_run_op(ctx, post_done)
  local utility = self._utility
