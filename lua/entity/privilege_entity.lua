-- Cloudsmith SDK Privilege entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PrivilegeEntity = {}
PrivilegeEntity.__index = PrivilegeEntity


function PrivilegeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PrivilegeEntity)
  self._name = "privilege"
  self._client = client
...
function PrivilegeEntity:get_name()
  return self._name
end


function PrivilegeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PrivilegeEntity.new(self._client, opts)
end
...
function PrivilegeEntity:mark_deleted()
  self._deleted = true
end


function PrivilegeEntity:deleted()
  return true == self._deleted
end


function PrivilegeEntity:data_set(args)
  if args ~= nil then
...
function PrivilegeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PrivilegeEntity:match_set(args)
  if args ~= nil then
...
function PrivilegeEntity:match_get()
...
function PrivilegeEntity:stream(action, args, callopts)
...
function PrivilegeEntity:_run_op(ctx, post_done)
  local utility = self._utility
