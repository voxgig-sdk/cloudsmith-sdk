...
local PackageEntity = {}
PackageEntity.__index = PackageEntity


function PackageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PackageEntity)
  self._name = "package"
  self._client = client
...
function PackageEntity:get_name()
  return self._name
end


function PackageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PackageEntity.new(self._client, opts)
end
...
function PackageEntity:mark_deleted()
  self._deleted = true
end


function PackageEntity:deleted()
  return true == self._deleted
end


function PackageEntity:data_set(args)
  if args ~= nil then
...
function PackageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PackageEntity:match_set(args)
  if args ~= nil then
...
function PackageEntity:match_get()
...
function PackageEntity:stream(action, args, callopts)
...
---@param reqmatch PackageLoadMatch
---@param ctrl? table
---@return Package
---@return string? err
function PackageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PackageListMatch
---@param ctrl? table
---@return Package[]
---@return string? err
function PackageEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata PackageCreateData
---@param ctrl? table
---@return Package
---@return string? err
function PackageEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch PackageRemoveMatch
---@param ctrl? table
---@return Package
---@return string? err
function PackageEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function PackageEntity:_run_op(ctx, post_done)
  local utility = self._utility
