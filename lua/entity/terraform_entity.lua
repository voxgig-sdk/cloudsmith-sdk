-- Cloudsmith SDK Terraform entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TerraformEntity = {}
TerraformEntity.__index = TerraformEntity


function TerraformEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TerraformEntity)
  self._name = "terraform"
  self._client = client
...
function TerraformEntity:get_name()
  return self._name
end


function TerraformEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TerraformEntity.new(self._client, opts)
end
...
function TerraformEntity:mark_deleted()
  self._deleted = true
end


function TerraformEntity:deleted()
  return true == self._deleted
end


function TerraformEntity:data_set(args)
  if args ~= nil then
...
function TerraformEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TerraformEntity:match_set(args)
  if args ~= nil then
...
function TerraformEntity:match_get()
...
function TerraformEntity:stream(action, args, callopts)
...
function TerraformEntity:_run_op(ctx, post_done)
  local utility = self._utility
