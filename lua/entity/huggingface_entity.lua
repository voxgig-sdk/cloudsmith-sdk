...
local HuggingfaceEntity = {}
HuggingfaceEntity.__index = HuggingfaceEntity


function HuggingfaceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HuggingfaceEntity)
...
function HuggingfaceEntity:get_name()
  return self._name
end


function HuggingfaceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HuggingfaceEntity.new(self._client, opts)
end
...
function HuggingfaceEntity:mark_deleted()
  self._deleted = true
end


function HuggingfaceEntity:deleted()
  return true == self._deleted
end


function HuggingfaceEntity:data_set(args)
  if args ~= nil then
...
function HuggingfaceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HuggingfaceEntity:match_set(args)
  if args ~= nil then
...
function HuggingfaceEntity:match_get()
...
function HuggingfaceEntity:stream(action, args, callopts)
...
---@param reqmatch HuggingfaceLoadMatch
---@param ctrl? table
---@return Huggingface
---@return string? err
function HuggingfaceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch HuggingfaceListMatch
---@param ctrl? table
---@return Huggingface[]
---@return string? err
function HuggingfaceEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata HuggingfaceCreateData
---@param ctrl? table
---@return Huggingface
---@return string? err
function HuggingfaceEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata HuggingfaceUpdateData
---@param ctrl? table
---@return Huggingface
---@return string? err
function HuggingfaceEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function HuggingfaceEntity:_run_op(ctx, post_done)
  local utility = self._utility
