-- Cloudsmith SDK TransferRegion entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TransferRegionEntity = {}
TransferRegionEntity.__index = TransferRegionEntity


function TransferRegionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TransferRegionEntity)
  self._name = "transfer_region"
  self._client = client
...
function TransferRegionEntity:get_name()
  return self._name
end


function TransferRegionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TransferRegionEntity.new(self._client, opts)
end
...
function TransferRegionEntity:mark_deleted()
  self._deleted = true
end


function TransferRegionEntity:deleted()
  return true == self._deleted
end


function TransferRegionEntity:data_set(args)
  if args ~= nil then
...
function TransferRegionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TransferRegionEntity:match_set(args)
  if args ~= nil then
...
function TransferRegionEntity:match_get()
...
function TransferRegionEntity:stream(action, args, callopts)
...
function TransferRegionEntity:_run_op(ctx, post_done)
  local utility = self._utility
