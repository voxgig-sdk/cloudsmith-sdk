-- Cloudsmith SDK Reset entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ResetEntity = {}
ResetEntity.__index = ResetEntity


function ResetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResetEntity)
  self._name = "reset"
  self._client = client
...
function ResetEntity:get_name()
  return self._name
end


function ResetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResetEntity.new(self._client, opts)
end
...
function ResetEntity:mark_deleted()
  self._deleted = true
end


function ResetEntity:deleted()
  return true == self._deleted
end


function ResetEntity:data_set(args)
  if args ~= nil then
...
function ResetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResetEntity:match_set(args)
  if args ~= nil then
...
function ResetEntity:match_get()
...
function ResetEntity:stream(action, args, callopts)
...
function ResetEntity:_run_op(ctx, post_done)
  local utility = self._utility
