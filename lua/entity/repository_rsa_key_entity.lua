-- Cloudsmith SDK RepositoryRsaKey entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RepositoryRsaKeyEntity = {}
RepositoryRsaKeyEntity.__index = RepositoryRsaKeyEntity


function RepositoryRsaKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RepositoryRsaKeyEntity)
  self._name = "repository_rsa_key"
  self._client = client
...
function RepositoryRsaKeyEntity:get_name()
  return self._name
end


function RepositoryRsaKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RepositoryRsaKeyEntity.new(self._client, opts)
end
...
function RepositoryRsaKeyEntity:mark_deleted()
  self._deleted = true
end


function RepositoryRsaKeyEntity:deleted()
  return true == self._deleted
end


function RepositoryRsaKeyEntity:data_set(args)
  if args ~= nil then
...
function RepositoryRsaKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RepositoryRsaKeyEntity:match_set(args)
  if args ~= nil then
...
function RepositoryRsaKeyEntity:match_get()
...
function RepositoryRsaKeyEntity:stream(action, args, callopts)
...
---@param reqmatch RepositoryRsaKeyLoadMatch
---@param ctrl? table
---@return RepositoryRsaKey
---@return string? err
function RepositoryRsaKeyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RepositoryRsaKeyCreateData
---@param ctrl? table
---@return RepositoryRsaKey
---@return string? err
function RepositoryRsaKeyEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function RepositoryRsaKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
