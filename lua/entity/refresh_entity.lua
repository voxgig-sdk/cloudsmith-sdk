-- Cloudsmith SDK Refresh entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RefreshEntity = {}
RefreshEntity.__index = RefreshEntity


function RefreshEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RefreshEntity)
  self._name = "refresh"
  self._client = client
...
function RefreshEntity:get_name()
  return self._name
end


function RefreshEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RefreshEntity.new(self._client, opts)
end
...
function RefreshEntity:mark_deleted()
  self._deleted = true
end


function RefreshEntity:deleted()
  return true == self._deleted
end


function RefreshEntity:data_set(args)
  if args ~= nil then
...
function RefreshEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RefreshEntity:match_set(args)
  if args ~= nil then
...
function RefreshEntity:match_get()
...
function RefreshEntity:stream(action, args, callopts)
...
function RefreshEntity:_run_op(ctx, post_done)
  local utility = self._utility
