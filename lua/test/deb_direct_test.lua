...
describe("DebDirect", function()
  it("should direct-list-deb", function()
    local setup = deb_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-deb", setup.live and "live" or "unit")
  ...
  it("should direct-load-deb", function()
    local setup = deb_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-deb", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function deb_direct_setup(mockres)
