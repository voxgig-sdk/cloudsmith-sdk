...
describe("SwiftDirect", function()
  it("should direct-list-swift", function()
    local setup = swift_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-swift", setup.live and "live" or "unit")
  ...
  it("should direct-load-swift", function()
    local setup = swift_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-swift", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function swift_direct_setup(mockres)
