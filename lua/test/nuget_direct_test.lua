...
describe("NugetDirect", function()
  it("should direct-list-nuget", function()
    local setup = nuget_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-nuget", setup.live and "live" or "unit")
  ...
  it("should direct-load-nuget", function()
    local setup = nuget_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-nuget", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function nuget_direct_setup(mockres)
