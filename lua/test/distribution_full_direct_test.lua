...
describe("DistributionFullDirect", function()
  it("should direct-list-distribution_full", function()
    local setup = distribution_full_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-distribution_full", setup.live and "live" or "unit")
  ...
  it("should direct-load-distribution_full", function()
    local setup = distribution_full_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-distribution_full", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function distribution_full_direct_setup(mockres)
