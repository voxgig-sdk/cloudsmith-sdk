...
describe("OrganizationGroupSyncEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:OrganizationGroupSync(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:OrganizationGroupSync(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:OrganizationGroupSync(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = organization_group_sync_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "organization_group_sync." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_GROUP_SYNC_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local organization_group_sync_ref01_ent = client:OrganizationGroupSync(nil)
    local organization_group_sync_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local organization_group_sync_ref01_match = {
...
function organization_group_sync_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/organization_group_sync/OrganizationGroupSyncTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read organization_group_sync test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_ORGANIZATION_GROUP_SYNC_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CLOUDSMITH_TEST_ORGANIZATION_GROUP_SYNC_ENTID"])
