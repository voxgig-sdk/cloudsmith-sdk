...
describe("ServiceEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_SERVICE_ENTID JSON to run live")
      return
    end
    ...
    local service_ref01_data = helpers.to_map(vs.getprop(
    ...
    assert.is_not_nil(service_ref01_data["id"])
    ...
    assert.are.equal(service_ref01_resdata_up0["id"], service_ref01_data_up0_up["id"])
    assert.are.equal(service_ref01_resdata_up0[service_ref01_markdef_up0_name], service_ref01_markdef_up0_value)

    -- LOAD
    local service_ref01_match_dt0 = {
      id = service_ref01_data["id"],
    }
    local service_ref01_data_dt0_loaded, err = service_ref01_ent:load(service_ref01_match_dt0, nil)
    assert.is_nil(err)
    local service_ref01_data_dt0_load_result = helpers.to_map(type(service_ref01_data_dt0_loaded) == 'table' and service_ref01_data_dt0_loaded.data_get and service_ref01_data_dt0_loaded:data_get() or service_ref01_data_dt0_loaded)
    assert.is_not_nil(service_ref01_data_dt0_load_result)
    assert.are.equal(service_ref01_data_dt0_load_result["id"], service_ref01_data["id"])
...
function service_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_SERVICE_ENTID")
