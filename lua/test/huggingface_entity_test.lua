...
describe("HuggingfaceEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Huggingface(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Huggingface(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Huggingface(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = huggingface_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "huggingface." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_HUGGINGFACE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local huggingface_ref01_ent = client:Huggingface(nil)
    local huggingface_ref01_data = helpers.to_map(vs.getprop(
    ...
    assert.is_not_nil(huggingface_ref01_data["id"])

    -- LIST
    local huggingface_ref01_match = {
    ...
    local huggingface_ref01_markdef_up0_name = "auth_mode"
    local huggingface_ref01_markdef_up0_value = "Mark01-huggingface_ref01_" .. tostring(setup.now)
    ...
    assert.are.equal(huggingface_ref01_resdata_up0["id"], huggingface_ref01_data_up0_up["id"])
    assert.are.equal(huggingface_ref01_resdata_up0[huggingface_ref01_markdef_up0_name], huggingface_ref01_markdef_up0_value)

    -- LOAD
    local huggingface_ref01_match_dt0 = {
      id = huggingface_ref01_data["id"],
    }
    local huggingface_ref01_data_dt0_loaded, err = huggingface_ref01_ent:load(huggingface_ref01_match_dt0, nil)
    assert.is_nil(err)
    local huggingface_ref01_data_dt0_load_result = helpers.to_map(type(huggingface_ref01_data_dt0_loaded) == 'table' and huggingface_ref01_data_dt0_loaded.data_get and huggingface_ref01_data_dt0_loaded:data_get() or huggingface_ref01_data_dt0_loaded)
    assert.is_not_nil(huggingface_ref01_data_dt0_load_result)
    assert.are.equal(huggingface_ref01_data_dt0_load_result["id"], huggingface_ref01_data["id"])
...
function huggingface_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/huggingface/HuggingfaceTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read huggingface test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_HUGGINGFACE_ENTID")
