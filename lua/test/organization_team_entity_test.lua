...
describe("OrganizationTeamEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:OrganizationTeam(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:OrganizationTeam(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:OrganizationTeam(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = organization_team_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "organization_team." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_TEAM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local organization_team_ref01_ent = client:OrganizationTeam(nil)
    local organization_team_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.organization_team"), "organization_team_ref01"))
    organization_team_ref01_data["org_id"] = setup.idmap["org01"]

    local organization_team_ref01_data_result, err = organization_team_ref01_ent:create(organization_team_ref01_data, nil)
    assert.is_nil(err)
    ...
    local organization_team_ref01_list_result, err = organization_team_ref01_ent:list(organization_team_ref01_match, nil)
    assert.is_nil(err)
    ...
    local organization_team_ref01_markdef_up0_name = "description"
    local organization_team_ref01_markdef_up0_value = "Mark01-organization_team_ref01_" .. tostring(setup.now)
    organization_team_ref01_data_up0_up[organization_team_ref01_markdef_up0_name] = organization_team_ref01_markdef_up0_value

    local organization_team_ref01_resdata_up0_result, err = organization_team_ref01_ent:update(organization_team_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local organization_team_ref01_resdata_up0 = helpers.to_map(organization_team_ref01_resdata_up0_result)
...
function organization_team_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/organization_team/OrganizationTeamTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read organization_team test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_ORGANIZATION_TEAM_ENTID")
