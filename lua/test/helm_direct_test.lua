...
describe("HelmDirect", function()
  it("should direct-list-helm", function()
    local setup = helm_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-helm", setup.live and "live" or "unit")
  ...
  it("should direct-load-helm", function()
    local setup = helm_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-helm", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function helm_direct_setup(mockres)
