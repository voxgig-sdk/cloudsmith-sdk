...
describe("TestEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_TEST_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local test_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.test")))
    local test_ref01_data = nil
    if #test_ref01_data_raw > 0 then
      test_ref01_data = helpers.to_map(test_ref01_data_raw[1][2])
    end
...
function test_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_TEST_ENTID")
