...
describe("RpmEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Rpm(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Rpm(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    assert.is_not_nil(rpm_ref01_data["id"])
    ...
    assert.is_nil(err)
    local rpm_ref01_resdata_up0 = helpers.to_map(type(rpm_ref01_resdata_up0_result) == 'table' and rpm_ref01_resdata_up0_result.data_get and rpm_ref01_resdata_up0_result:data_get() or rpm_ref01_resdata_up0_result)
    assert.is_not_nil(rpm_ref01_resdata_up0)
    assert.are.equal(rpm_ref01_resdata_up0["id"], rpm_ref01_data_up0_up["id"])
    ...
    assert.is_nil(err)
    local rpm_ref01_data_dt0_load_result = helpers.to_map(type(rpm_ref01_data_dt0_loaded) == 'table' and rpm_ref01_data_dt0_loaded.data_get and rpm_ref01_data_dt0_loaded:data_get() or rpm_ref01_data_dt0_loaded)
    assert.is_not_nil(rpm_ref01_data_dt0_load_result)
    assert.are.equal(rpm_ref01_data_dt0_load_result["id"], rpm_ref01_data["id"])
