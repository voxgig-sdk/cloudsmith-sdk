...
describe("MavenDirect", function()
  it("should direct-list-maven", function()
    local setup = maven_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-maven", setup.live and "live" or "unit")
  ...
  it("should direct-load-maven", function()
    local setup = maven_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-maven", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function maven_direct_setup(mockres)
