...
describe("DynamicMappingDirect", function()
  it("should direct-list-dynamic_mapping", function()
    local setup = dynamic_mapping_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-dynamic_mapping", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org01", "provider_setting01"}) do
  ...
  it("should direct-load-dynamic_mapping", function()
    local setup = dynamic_mapping_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-dynamic_mapping", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function dynamic_mapping_direct_setup(mockres)
