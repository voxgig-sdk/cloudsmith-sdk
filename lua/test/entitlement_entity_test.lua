...
describe("EntitlementEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Entitlement(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = entitlement_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "entitlement." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ENTITLEMENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local entitlement_ref01_ent = client:Entitlement(nil)
    local entitlement_ref01_data = helpers.to_map(vs.getprop(
...
function entitlement_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/entitlement/EntitlementTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read entitlement test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_ENTITLEMENT_ENTID")
