...
describe("PackageDenyPolicyDirect", function()
  it("should direct-list-package_deny_policy", function()
    local setup = package_deny_policy_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-package_deny_policy", setup.live and "live" or "unit")
  ...
  it("should direct-load-package_deny_policy", function()
    local setup = package_deny_policy_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-package_deny_policy", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function package_deny_policy_direct_setup(mockres)
