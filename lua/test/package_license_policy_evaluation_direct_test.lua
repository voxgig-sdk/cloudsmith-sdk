-- PackageLicensePolicyEvaluation direct test
...
describe("PackageLicensePolicyEvaluationDirect", function()
  it("should direct-list-package_license_policy_evaluation", function()
    local setup = package_license_policy_evaluation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-package_license_policy_evaluation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org01", "policy_slug_perm01"}) do
    ...
    if setup.live then
      ...
      params["policy_slug_perm"] = "direct01"
    end
  ...
  it("should direct-load-package_license_policy_evaluation", function()
    local setup = package_license_policy_evaluation_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-package_license_policy_evaluation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function package_license_policy_evaluation_direct_setup(mockres)
