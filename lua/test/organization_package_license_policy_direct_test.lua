-- OrganizationPackageLicensePolicy direct test
...
describe("OrganizationPackageLicensePolicyDirect", function()
  it("should direct-list-organization_package_license_policy", function()
    local setup = organization_package_license_policy_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_package_license_policy", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org01"}) do
  ...
  it("should direct-load-organization_package_license_policy", function()
    local setup = organization_package_license_policy_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-organization_package_license_policy", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function organization_package_license_policy_direct_setup(mockres)
