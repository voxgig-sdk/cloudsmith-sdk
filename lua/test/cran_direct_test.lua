...
describe("CranDirect", function()
  it("should direct-list-cran", function()
    local setup = cran_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-cran", setup.live and "live" or "unit")
  ...
  it("should direct-load-cran", function()
    local setup = cran_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-cran", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function cran_direct_setup(mockres)
