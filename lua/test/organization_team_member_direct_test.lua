-- OrganizationTeamMember direct test
...
describe("OrganizationTeamMemberDirect", function()
  it("should direct-list-organization_team_member", function()
    local setup = organization_team_member_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_team_member", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"org01", "team01"}) do
...
function organization_team_member_direct_setup(mockres)
