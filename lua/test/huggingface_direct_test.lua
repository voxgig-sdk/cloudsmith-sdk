...
describe("HuggingfaceDirect", function()
  it("should direct-list-huggingface", function()
    local setup = huggingface_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-huggingface", setup.live and "live" or "unit")
  ...
  it("should direct-load-huggingface", function()
    local setup = huggingface_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-huggingface", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function huggingface_direct_setup(mockres)
