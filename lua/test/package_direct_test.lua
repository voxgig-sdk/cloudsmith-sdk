...
describe("PackageDirect", function()
  it("should direct-list-package", function()
    local setup = package_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-package", setup.live and "live" or "unit")
  ...
  it("should direct-load-package", function()
    local setup = package_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-package", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function package_direct_setup(mockres)
