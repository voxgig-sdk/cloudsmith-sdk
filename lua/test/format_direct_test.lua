...
describe("FormatDirect", function()
  it("should direct-list-format", function()
    local setup = format_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-format", setup.live and "live" or "unit")
  ...
  it("should direct-load-format", function()
    local setup = format_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-format", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function format_direct_setup(mockres)
