...
describe("NugetEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Nuget(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Nuget(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_NUGET_ENTID JSON to run live")
      return
    end
    ...
    local nuget_ref01_data = helpers.to_map(vs.getprop(
    ...
    local nuget_ref01_data_result, err = nuget_ref01_ent:create(nuget_ref01_data, nil)
    assert.is_nil(err)
    ...
    local nuget_ref01_list_result, err = nuget_ref01_ent:list(nuget_ref01_match, nil)
    assert.is_nil(err)
    ...
    local nuget_ref01_markdef_up0_name = "auth_mode"
    local nuget_ref01_markdef_up0_value = "Mark01-nuget_ref01_" .. tostring(setup.now)
...
function nuget_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_NUGET_ENTID")
