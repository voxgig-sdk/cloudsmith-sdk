...
describe("OrganizationInviteEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:OrganizationInvite(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:OrganizationInvite(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:OrganizationInvite(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = organization_invite_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "organization_invite." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local organization_invite_ref01_ent = client:OrganizationInvite(nil)
    local organization_invite_ref01_data = helpers.to_map(vs.getprop(
    ...
    assert.is_not_nil(organization_invite_ref01_data["id"])

    -- LIST
    local organization_invite_ref01_match = {
    ...
    local organization_invite_ref01_markdef_up0_name = "email"
    local organization_invite_ref01_markdef_up0_value = "Mark01-organization_invite_ref01_" .. tostring(setup.now)
    ...
    assert.are.equal(organization_invite_ref01_resdata_up0["id"], organization_invite_ref01_data_up0_up["id"])
...
function organization_invite_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/organization_invite/OrganizationInviteTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read organization_invite test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID"])
