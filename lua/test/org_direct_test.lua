...
describe("OrgDirect", function()
  it("should direct-list-org", function()
    local setup = org_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-org", setup.live and "live" or "unit")
  ...
  it("should direct-load-org", function()
    local setup = org_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-org", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function org_direct_setup(mockres)
