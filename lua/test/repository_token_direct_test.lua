...
describe("RepositoryTokenDirect", function()
  it("should direct-list-repository_token", function()
    local setup = repository_token_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-repository_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"owner01", "repo01"}) do
  ...
  it("should direct-load-repository_token", function()
    local setup = repository_token_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-repository_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function repository_token_direct_setup(mockres)
