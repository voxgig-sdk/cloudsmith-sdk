...
describe("PackageLicensePolicyEvaluationEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:PackageLicensePolicyEvaluation(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:PackageLicensePolicyEvaluation(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:PackageLicensePolicyEvaluation(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = package_license_policy_evaluation_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "package_license_policy_evaluation." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_PACKAGE_LICENSE_POLICY_EVALUATION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local package_license_policy_evaluation_ref01_ent = client:PackageLicensePolicyEvaluation(nil)
    local package_license_policy_evaluation_ref01_data = helpers.to_map(vs.getprop(
...
function package_license_policy_evaluation_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/package_license_policy_evaluation/PackageLicensePolicyEvaluationTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read package_license_policy_evaluation test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_PACKAGE_LICENSE_POLICY_EVALUATION_ENTID")
