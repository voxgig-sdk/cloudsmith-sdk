...
describe("RepositoryTokenEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:RepositoryToken(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:RepositoryToken(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    assert.is_not_nil(repository_token_ref01_data["id"])
    ...
    assert.are.equal(repository_token_ref01_resdata_up0["id"], repository_token_ref01_data_up0_up["id"])
    ...
    assert.is_nil(err)
    local repository_token_ref01_data_dt0_load_result = helpers.to_map(type(repository_token_ref01_data_dt0_loaded) == 'table' and repository_token_ref01_data_dt0_loaded.data_get and repository_token_ref01_data_dt0_loaded:data_get() or repository_token_ref01_data_dt0_loaded)
    assert.is_not_nil(repository_token_ref01_data_dt0_load_result)
    assert.are.equal(repository_token_ref01_data_dt0_load_result["id"], repository_token_ref01_data["id"])
