...
describe("ComposerDirect", function()
  it("should direct-list-composer", function()
    local setup = composer_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-composer", setup.live and "live" or "unit")
  ...
  it("should direct-load-composer", function()
    local setup = composer_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-composer", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function composer_direct_setup(mockres)
