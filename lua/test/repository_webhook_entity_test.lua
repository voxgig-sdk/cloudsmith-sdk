...
describe("RepositoryWebhookEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:RepositoryWebhook(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:RepositoryWebhook(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:RepositoryWebhook(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_REPOSITORY_WEBHOOK_ENTID JSON to run live")
      return
    end
    ...
    local repository_webhook_ref01_data_result, err = repository_webhook_ref01_ent:create(repository_webhook_ref01_data, nil)
    assert.is_nil(err)
    ...
    local repository_webhook_ref01_list_result, err = repository_webhook_ref01_ent:list(repository_webhook_ref01_match, nil)
    assert.is_nil(err)
    ...
    local repository_webhook_ref01_resdata_up0_result, err = repository_webhook_ref01_ent:update(repository_webhook_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local repository_webhook_ref01_resdata_up0 = helpers.to_map(repository_webhook_ref01_resdata_up0_result)
...
function repository_webhook_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/repository_webhook/RepositoryWebhookTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read repository_webhook test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_REPOSITORY_WEBHOOK_ENTID")
