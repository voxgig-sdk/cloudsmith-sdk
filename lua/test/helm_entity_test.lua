...
describe("HelmEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Helm(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Helm(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Helm(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = helm_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "helm." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_HELM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local helm_ref01_ent = client:Helm(nil)
    local helm_ref01_data = helpers.to_map(vs.getprop(
    ...
    local helm_ref01_markdef_up0_name = "auth_mode"
    local helm_ref01_markdef_up0_value = "Mark01-helm_ref01_" .. tostring(setup.now)
...
function helm_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/helm/HelmTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read helm test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_HELM_ENTID")
