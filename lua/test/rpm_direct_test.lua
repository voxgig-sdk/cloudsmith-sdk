...
describe("RpmDirect", function()
  it("should direct-list-rpm", function()
    local setup = rpm_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-rpm", setup.live and "live" or "unit")
  ...
  it("should direct-load-rpm", function()
    local setup = rpm_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-rpm", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function rpm_direct_setup(mockres)
