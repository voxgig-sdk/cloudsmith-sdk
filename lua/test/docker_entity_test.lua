...
describe("DockerEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Docker(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Docker(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Docker(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = docker_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "docker." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_DOCKER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local docker_ref01_ent = client:Docker(nil)
    local docker_ref01_data = helpers.to_map(vs.getprop(
    ...
    assert.is_not_nil(docker_ref01_data["id"])

    -- LIST
    local docker_ref01_match = {
    ...
    local docker_ref01_markdef_up0_name = "auth_mode"
    local docker_ref01_markdef_up0_value = "Mark01-docker_ref01_" .. tostring(setup.now)
    ...
    assert.are.equal(docker_ref01_resdata_up0["id"], docker_ref01_data_up0_up["id"])
    assert.are.equal(docker_ref01_resdata_up0[docker_ref01_markdef_up0_name], docker_ref01_markdef_up0_value)

    -- LOAD
    local docker_ref01_match_dt0 = {
      id = docker_ref01_data["id"],
    }
    local docker_ref01_data_dt0_loaded, err = docker_ref01_ent:load(docker_ref01_match_dt0, nil)
    assert.is_nil(err)
    local docker_ref01_data_dt0_load_result = helpers.to_map(type(docker_ref01_data_dt0_loaded) == 'table' and docker_ref01_data_dt0_loaded.data_get and docker_ref01_data_dt0_loaded:data_get() or docker_ref01_data_dt0_loaded)
    assert.is_not_nil(docker_ref01_data_dt0_load_result)
    assert.are.equal(docker_ref01_data_dt0_load_result["id"], docker_ref01_data["id"])
...
function docker_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/docker/DockerTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read docker test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_DOCKER_ENTID")
