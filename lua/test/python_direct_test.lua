...
describe("PythonDirect", function()
  it("should direct-list-python", function()
    local setup = python_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-python", setup.live and "live" or "unit")
  ...
  it("should direct-load-python", function()
    local setup = python_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-python", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function python_direct_setup(mockres)
