...
describe("HexDirect", function()
  it("should direct-list-hex", function()
    local setup = hex_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-hex", setup.live and "live" or "unit")
  ...
  it("should direct-load-hex", function()
    local setup = hex_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-hex", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function hex_direct_setup(mockres)
