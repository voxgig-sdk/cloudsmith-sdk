...
describe("NpmDirect", function()
  it("should direct-list-npm", function()
    local setup = npm_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-npm", setup.live and "live" or "unit")
  ...
  it("should direct-load-npm", function()
    local setup = npm_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-npm", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function npm_direct_setup(mockres)
