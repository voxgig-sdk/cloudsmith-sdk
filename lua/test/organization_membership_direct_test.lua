-- OrganizationMembership direct test
...
describe("OrganizationMembershipDirect", function()
  it("should direct-list-organization_membership", function()
    local setup = organization_membership_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_membership", setup.live and "live" or "unit")
  ...
  it("should direct-load-organization_membership", function()
    local setup = organization_membership_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-organization_membership", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function organization_membership_direct_setup(mockres)
