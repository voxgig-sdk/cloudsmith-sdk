...
describe("RepoDirect", function()
  it("should direct-list-repo", function()
    local setup = repo_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-repo", setup.live and "live" or "unit")
  ...
  it("should direct-load-repo", function()
    local setup = repo_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-repo", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function repo_direct_setup(mockres)
