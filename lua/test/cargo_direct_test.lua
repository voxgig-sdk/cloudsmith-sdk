...
describe("CargoDirect", function()
  it("should direct-list-cargo", function()
    local setup = cargo_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-cargo", setup.live and "live" or "unit")
  ...
  it("should direct-load-cargo", function()
    local setup = cargo_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-cargo", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function cargo_direct_setup(mockres)
