...
describe("DockerDirect", function()
  it("should direct-list-docker", function()
    local setup = docker_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-docker", setup.live and "live" or "unit")
  ...
  it("should direct-load-docker", function()
    local setup = docker_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-docker", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function docker_direct_setup(mockres)
