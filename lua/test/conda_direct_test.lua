...
describe("CondaDirect", function()
  it("should direct-list-conda", function()
    local setup = conda_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-conda", setup.live and "live" or "unit")
  ...
  it("should direct-load-conda", function()
    local setup = conda_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-conda", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function conda_direct_setup(mockres)
