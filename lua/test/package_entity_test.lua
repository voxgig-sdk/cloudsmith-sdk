...
describe("PackageEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Package(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Package(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Package(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = package_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "package." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_PACKAGE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local package_ref01_ent = client:Package(nil)
    local package_ref01_data = helpers.to_map(vs.getprop(
    ...
    assert.is_not_nil(package_ref01_data["id"])

    -- LIST
    local package_ref01_match = {
      ["identifier"] = setup.idmap["identifier01"],
      ["owner"] = setup.idmap["owner01"],
      ["repo"] = setup.idmap["repo01"],
    }

    local package_ref01_list_result, err = package_ref01_ent:list(package_ref01_match, nil)
    assert.is_nil(err)
    ...
    assert.is_nil(err)
    local package_ref01_data_dt0_load_result = helpers.to_map(type(package_ref01_data_dt0_loaded) == 'table' and package_ref01_data_dt0_loaded.data_get and package_ref01_data_dt0_loaded:data_get() or package_ref01_data_dt0_loaded)
    assert.is_not_nil(package_ref01_data_dt0_load_result)
    assert.are.equal(package_ref01_data_dt0_load_result["id"], package_ref01_data["id"])
...
function package_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/package/PackageTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read package test data: " .. entity_data_file)
  end
  ...
  local idmap = vs.transform(
    { "package01", "package02", "package03", "identifier01", "owner01", "repo01" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_PACKAGE_ENTID")
