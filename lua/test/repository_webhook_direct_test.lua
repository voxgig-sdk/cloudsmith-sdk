...
describe("RepositoryWebhookDirect", function()
  it("should direct-list-repository_webhook", function()
    local setup = repository_webhook_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-repository_webhook", setup.live and "live" or "unit")
  ...
  it("should direct-load-repository_webhook", function()
    local setup = repository_webhook_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-repository_webhook", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function repository_webhook_direct_setup(mockres)
