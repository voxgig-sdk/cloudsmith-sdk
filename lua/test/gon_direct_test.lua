...
describe("GonDirect", function()
  it("should direct-list-gon", function()
    local setup = gon_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-gon", setup.live and "live" or "unit")
  ...
  it("should direct-load-gon", function()
    local setup = gon_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-gon", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function gon_direct_setup(mockres)
