...
describe("PythonEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Python(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Python(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Python(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = python_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "python." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_PYTHON_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local python_ref01_ent = client:Python(nil)
    local python_ref01_data = helpers.to_map(vs.getprop(
    ...
    local python_ref01_data_result, err = python_ref01_ent:create(python_ref01_data, nil)
    assert.is_nil(err)
    ...
    local python_ref01_list_result, err = python_ref01_ent:list(python_ref01_match, nil)
    assert.is_nil(err)
    ...
    local python_ref01_markdef_up0_name = "auth_mode"
    local python_ref01_markdef_up0_value = "Mark01-python_ref01_" .. tostring(setup.now)
    python_ref01_data_up0_up[python_ref01_markdef_up0_name] = python_ref01_markdef_up0_value

    local python_ref01_resdata_up0_result, err = python_ref01_ent:update(python_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local python_ref01_resdata_up0 = helpers.to_map(python_ref01_resdata_up0_result)
...
function python_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/python/PythonTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read python test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CLOUDSMITH_TEST_PYTHON_ENTID")
