...
class OrganizationTeamMemberDirectTest < Minitest::Test
  def test_direct_list_organization_team_member
    setup = organization_team_member_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-organization_team_member", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["org01", "team01"].each do |_live_key|
...
def organization_team_member_direct_setup(mockres)
