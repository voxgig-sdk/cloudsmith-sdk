...
class CargoEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Cargo(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Cargo(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = cargo_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "cargo." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_CARGO_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    cargo_ref01_ent = client.Cargo(nil)
    ...
    assert !cargo_ref01_data["id"].nil?
    ...
    assert_equal cargo_ref01_resdata_up0["id"], cargo_ref01_data_up0_up["id"]
    ...
    cargo_ref01_data_dt0_loaded = cargo_ref01_ent.load(cargo_ref01_match_dt0, nil)
    cargo_ref01_data_dt0_load_result = Helpers.to_map(cargo_ref01_data_dt0_loaded.respond_to?(:data_get) ? cargo_ref01_data_dt0_loaded.data_get : cargo_ref01_data_dt0_loaded)
    assert !cargo_ref01_data_dt0_load_result.nil?
    assert_equal cargo_ref01_data_dt0_load_result["id"], cargo_ref01_data["id"]

  end
end

def cargo_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "cargo", "CargoTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_CARGO_ENTID"]
