...
class RepositoryTokenDirectTest < Minitest::Test
  def test_direct_list_repository_token
    setup = repository_token_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-repository_token", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["owner01", "repo01"].each do |_live_key|
  ...
  def test_direct_load_repository_token
    setup = repository_token_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-repository_token", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def repository_token_direct_setup(mockres)
