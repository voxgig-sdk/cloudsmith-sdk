...
class UserAuthenticationTokenEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = user_authentication_token_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "update"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "user_authentication_token." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_USER_AUTHENTICATION_TOKEN_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    user_authentication_token_ref01_ent = client.UserAuthenticationToken(nil)
...
def user_authentication_token_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "user_authentication_token", "UserAuthenticationTokenTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_USER_AUTHENTICATION_TOKEN_ENTID"]
