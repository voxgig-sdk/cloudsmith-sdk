...
class OrganizationInviteEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.OrganizationInvite(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.OrganizationInvite(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = organization_invite_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "organization_invite." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    organization_invite_ref01_ent = client.OrganizationInvite(nil)
    ...
    assert !organization_invite_ref01_data["id"].nil?
    ...
    assert_equal organization_invite_ref01_resdata_up0["id"], organization_invite_ref01_data_up0_up["id"]
...
def organization_invite_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "organization_invite", "OrganizationInviteTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID"]
