...
class PackageLicensePolicyEvaluationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.PackageLicensePolicyEvaluation(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.PackageLicensePolicyEvaluation(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = package_license_policy_evaluation_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "package_license_policy_evaluation." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_PACKAGE_LICENSE_POLICY_EVALUATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    package_license_policy_evaluation_ref01_ent = client.PackageLicensePolicyEvaluation(nil)
    ...
    assert !package_license_policy_evaluation_ref01_data["id"].nil?
    ...
    package_license_policy_evaluation_ref01_data_dt0_loaded = package_license_policy_evaluation_ref01_ent.load(package_license_policy_evaluation_ref01_match_dt0, nil)
    package_license_policy_evaluation_ref01_data_dt0_load_result = Helpers.to_map(package_license_policy_evaluation_ref01_data_dt0_loaded.respond_to?(:data_get) ? package_license_policy_evaluation_ref01_data_dt0_loaded.data_get : package_license_policy_evaluation_ref01_data_dt0_loaded)
    assert !package_license_policy_evaluation_ref01_data_dt0_load_result.nil?
    assert_equal package_license_policy_evaluation_ref01_data_dt0_load_result["id"], package_license_policy_evaluation_ref01_data["id"]

  end
end

def package_license_policy_evaluation_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "package_license_policy_evaluation", "PackageLicensePolicyEvaluationTestData.json")
  ...
  idmap = Vs.transform(
    ["package_license_policy_evaluation01", "package_license_policy_evaluation02", "package_license_policy_evaluation03", "org01", "org02", "org03", "license_policy01", "license_policy02", "license_policy03", "policy_slug_perm01"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_PACKAGE_LICENSE_POLICY_EVALUATION_ENTID"]
