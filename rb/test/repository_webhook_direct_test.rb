...
class RepositoryWebhookDirectTest < Minitest::Test
  def test_direct_list_repository_webhook
    setup = repository_webhook_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-repository_webhook", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_repository_webhook
    setup = repository_webhook_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-repository_webhook", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def repository_webhook_direct_setup(mockres)
