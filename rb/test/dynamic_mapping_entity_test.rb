...
class DynamicMappingEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DynamicMapping(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DynamicMapping(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = dynamic_mapping_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "dynamic_mapping." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_DYNAMIC_MAPPING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    dynamic_mapping_ref01_data = nil
    if dynamic_mapping_ref01_data_raw.length > 0
      dynamic_mapping_ref01_data = Helpers.to_map(dynamic_mapping_ref01_data_raw[0][1])
    end

    # LIST
    dynamic_mapping_ref01_ent = client.DynamicMapping(nil)
    dynamic_mapping_ref01_match = {
      "org_id" => setup[:idmap]["org01"],
      "provider_setting" => setup[:idmap]["provider_setting01"],
    }

    dynamic_mapping_ref01_list_result = dynamic_mapping_ref01_ent.list(dynamic_mapping_ref01_match, nil)
...
def dynamic_mapping_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "dynamic_mapping", "DynamicMappingTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_DYNAMIC_MAPPING_ENTID"]
