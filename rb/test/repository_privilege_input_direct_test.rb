...
class RepositoryPrivilegeInputDirectTest < Minitest::Test
  def test_direct_list_repository_privilege_input
    setup = repository_privilege_input_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-repository_privilege_input", setup[:live] ? "live" : "unit")
    if _should_skip
...
def repository_privilege_input_direct_setup(mockres)
