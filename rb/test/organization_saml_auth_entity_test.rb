...
class OrganizationSamlAuthEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = organization_saml_auth_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "organization_saml_auth." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_SAML_AUTH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    organization_saml_auth_ref01_data = nil
    if organization_saml_auth_ref01_data_raw.length > 0
      organization_saml_auth_ref01_data = Helpers.to_map(organization_saml_auth_ref01_data_raw[0][1])
    end

    # UPDATE
    organization_saml_auth_ref01_ent = client.OrganizationSamlAuth(nil)
    ...
    organization_saml_auth_ref01_data_dt0_loaded = organization_saml_auth_ref01_ent.load(organization_saml_auth_ref01_match_dt0, nil)
    assert !organization_saml_auth_ref01_data_dt0_loaded.nil?

  end
end

def organization_saml_auth_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "organization_saml_auth", "OrganizationSamlAuthTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_ORGANIZATION_SAML_AUTH_ENTID"]
