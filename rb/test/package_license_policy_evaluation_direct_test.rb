...
class PackageLicensePolicyEvaluationDirectTest < Minitest::Test
  def test_direct_list_package_license_policy_evaluation
    setup = package_license_policy_evaluation_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-package_license_policy_evaluation", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["org01", "policy_slug_perm01"].each do |_live_key|
  ...
  def test_direct_load_package_license_policy_evaluation
    setup = package_license_policy_evaluation_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-package_license_policy_evaluation", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def package_license_policy_evaluation_direct_setup(mockres)
