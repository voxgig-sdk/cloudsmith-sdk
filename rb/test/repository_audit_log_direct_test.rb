...
class RepositoryAuditLogDirectTest < Minitest::Test
  def test_direct_list_repository_audit_log
    setup = repository_audit_log_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-repository_audit_log", setup[:live] ? "live" : "unit")
    if _should_skip
...
def repository_audit_log_direct_setup(mockres)
