...
class OrganizationInviteDirectTest < Minitest::Test
  def test_direct_list_organization_invite
    setup = organization_invite_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-organization_invite", setup[:live] ? "live" : "unit")
    if _should_skip
...
def organization_invite_direct_setup(mockres)
