...
class RubyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Ruby(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Ruby(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    assert_equal ruby_ref01_resdata_up0["id"], ruby_ref01_data_up0_up["id"]
    ...
    ruby_ref01_data_dt0_load_result = Helpers.to_map(ruby_ref01_data_dt0_loaded.respond_to?(:data_get) ? ruby_ref01_data_dt0_loaded.data_get : ruby_ref01_data_dt0_loaded)
    assert !ruby_ref01_data_dt0_load_result.nil?
    assert_equal ruby_ref01_data_dt0_load_result["id"], ruby_ref01_data["id"]
