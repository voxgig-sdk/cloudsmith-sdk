...
class RepositoryTokenEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.RepositoryToken(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.RepositoryToken(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    assert !repository_token_ref01_data["id"].nil?
    ...
    assert_equal repository_token_ref01_resdata_up0["id"], repository_token_ref01_data_up0_up["id"]
    ...
    repository_token_ref01_data_dt0_load_result = Helpers.to_map(repository_token_ref01_data_dt0_loaded.respond_to?(:data_get) ? repository_token_ref01_data_dt0_loaded.data_get : repository_token_ref01_data_dt0_loaded)
    assert !repository_token_ref01_data_dt0_load_result.nil?
    assert_equal repository_token_ref01_data_dt0_load_result["id"], repository_token_ref01_data["id"]
