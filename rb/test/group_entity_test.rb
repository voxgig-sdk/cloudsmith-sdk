...
class GroupEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_GROUP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    group_ref01_data = nil
    if group_ref01_data_raw.length > 0
...
def group_basic_setup(extra)
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_GROUP_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["CLOUDSMITH_TEST_GROUP_ENTID"])
