...
class DartEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Dart(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Dart(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = dart_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "dart." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_DART_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    dart_ref01_ent = client.Dart(nil)
    dart_ref01_data = Helpers.to_map(Vs.getprop(
    ...
    dart_ref01_markdef_up0_name = "auth_mode"
    dart_ref01_markdef_up0_value = "Mark01-dart_ref01_#{setup[:now]}"
    ...
    assert_equal dart_ref01_resdata_up0["id"], dart_ref01_data_up0_up["id"]
    ...
    dart_ref01_data_dt0_loaded = dart_ref01_ent.load(dart_ref01_match_dt0, nil)
    dart_ref01_data_dt0_load_result = Helpers.to_map(dart_ref01_data_dt0_loaded.respond_to?(:data_get) ? dart_ref01_data_dt0_loaded.data_get : dart_ref01_data_dt0_loaded)
    assert !dart_ref01_data_dt0_load_result.nil?
    assert_equal dart_ref01_data_dt0_load_result["id"], dart_ref01_data["id"]

  end
end

def dart_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "dart", "DartTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_DART_ENTID"]
