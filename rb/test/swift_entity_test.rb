...
class SwiftEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Swift(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Swift(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = swift_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "swift." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_SWIFT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    swift_ref01_ent = client.Swift(nil)
    ...
    assert !swift_ref01_data["id"].nil?
    ...
    assert_equal swift_ref01_resdata_up0["id"], swift_ref01_data_up0_up["id"]
    ...
    swift_ref01_data_dt0_loaded = swift_ref01_ent.load(swift_ref01_match_dt0, nil)
    swift_ref01_data_dt0_load_result = Helpers.to_map(swift_ref01_data_dt0_loaded.respond_to?(:data_get) ? swift_ref01_data_dt0_loaded.data_get : swift_ref01_data_dt0_loaded)
    assert !swift_ref01_data_dt0_load_result.nil?
    assert_equal swift_ref01_data_dt0_load_result["id"], swift_ref01_data["id"]

  end
end

def swift_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "swift", "SwiftTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_SWIFT_ENTID"]
