...
class OrganizationPackageLicensePolicyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.OrganizationPackageLicensePolicy(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.OrganizationPackageLicensePolicy(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = organization_package_license_policy_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "organization_package_license_policy." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_PACKAGE_LICENSE_POLICY_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    organization_package_license_policy_ref01_ent = client.OrganizationPackageLicensePolicy(nil)
...
def organization_package_license_policy_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "organization_package_license_policy", "OrganizationPackageLicensePolicyTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_ORGANIZATION_PACKAGE_LICENSE_POLICY_ENTID"]
