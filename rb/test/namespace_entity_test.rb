...
class NamespaceEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Namespace(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Namespace(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_NAMESPACE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    namespace_ref01_list_result = namespace_ref01_ent.list(namespace_ref01_match, nil)
...
def namespace_basic_setup(extra)
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_NAMESPACE_ENTID"]
