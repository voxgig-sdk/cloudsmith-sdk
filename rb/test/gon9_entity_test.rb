...
class Gon9EntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = gon9_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["update"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "gon9." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    gon9_ref01_data = nil
    if gon9_ref01_data_raw.length > 0
      gon9_ref01_data = Helpers.to_map(gon9_ref01_data_raw[0][1])
    end

    # UPDATE
    gon9_ref01_ent = client.Gon9(nil)
    ...
    gon9_ref01_resdata_up0_result = gon9_ref01_ent.update(gon9_ref01_data_up0_up, nil)
    gon9_ref01_resdata_up0 = Helpers.to_map(gon9_ref01_resdata_up0_result)
    assert !gon9_ref01_resdata_up0.nil?
    assert_equal gon9_ref01_resdata_up0[gon9_ref01_markdef_up0_name], gon9_ref01_markdef_up0_value

  end
end

def gon9_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "gon9", "Gon9TestData.json")
