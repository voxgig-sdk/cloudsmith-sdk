...
class OrganizationGroupSyncDirectTest < Minitest::Test
  def test_direct_list_organization_group_sync
    setup = organization_group_sync_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-organization_group_sync", setup[:live] ? "live" : "unit")
    if _should_skip
...
def organization_group_sync_direct_setup(mockres)
