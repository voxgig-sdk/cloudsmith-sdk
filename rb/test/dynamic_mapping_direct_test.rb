...
class DynamicMappingDirectTest < Minitest::Test
  def test_direct_list_dynamic_mapping
    setup = dynamic_mapping_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-dynamic_mapping", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["org01", "provider_setting01"].each do |_live_key|
  ...
  def test_direct_load_dynamic_mapping
    setup = dynamic_mapping_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-dynamic_mapping", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def dynamic_mapping_direct_setup(mockres)
