...
class PackageDenyPolicyEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.PackageDenyPolicy(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.PackageDenyPolicy(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    assert !package_deny_policy_ref01_data["id"].nil?
    ...
    assert_equal package_deny_policy_ref01_resdata_up0["id"], package_deny_policy_ref01_data_up0_up["id"]
    ...
    package_deny_policy_ref01_data_dt0_load_result = Helpers.to_map(package_deny_policy_ref01_data_dt0_loaded.respond_to?(:data_get) ? package_deny_policy_ref01_data_dt0_loaded.data_get : package_deny_policy_ref01_data_dt0_loaded)
    assert !package_deny_policy_ref01_data_dt0_load_result.nil?
    assert_equal package_deny_policy_ref01_data_dt0_load_result["id"], package_deny_policy_ref01_data["id"]
