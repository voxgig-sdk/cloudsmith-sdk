...
class OrganizationMembershipEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.OrganizationMembership(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.OrganizationMembership(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    organization_membership_ref01_resdata_up0 = Helpers.to_map(organization_membership_ref01_resdata_up0_result.respond_to?(:data_get) ? organization_membership_ref01_resdata_up0_result.data_get : organization_membership_ref01_resdata_up0_result)
