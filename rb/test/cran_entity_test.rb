...
class CranEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Cran(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Cran(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = cran_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "cran." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_CRAN_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    cran_ref01_ent = client.Cran(nil)
...
def cran_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "cran", "CranTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_CRAN_ENTID"]
