...
class RepositoryTokenRefreshEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = repository_token_refresh_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "repository_token_refresh." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_REPOSITORY_TOKEN_REFRESH_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    repository_token_refresh_ref01_ent = client.RepositoryTokenRefresh(nil)
...
def repository_token_refresh_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "repository_token_refresh", "RepositoryTokenRefreshTestData.json")
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_REPOSITORY_TOKEN_REFRESH_ENTID"]
