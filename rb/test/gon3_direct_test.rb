...
class Gon3DirectTest < Minitest::Test
  def test_direct_list_gon3
    setup = gon3_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-gon3", setup[:live] ? "live" : "unit")
    if _should_skip
