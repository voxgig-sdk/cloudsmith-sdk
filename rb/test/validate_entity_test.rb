...
class ValidateEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_VALIDATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    validate_ref01_data = nil
    if validate_ref01_data_raw.length > 0
...
def validate_basic_setup(extra)
  ...
  entid_env_raw = ENV["CLOUDSMITH_TEST_VALIDATE_ENTID"]
