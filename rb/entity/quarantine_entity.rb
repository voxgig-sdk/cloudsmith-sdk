...
class QuarantineEntity
  ...
  def make
    opts = @_entopts.dup
    QuarantineEntity.new(@_client, opts)
  end
  ...
  # @return [Quarantine, Hash] the current Quarantine data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Quarantine fields)
  def match_get
