...
class UserProfileEntity
  ...
  def make
    opts = @_entopts.dup
    UserProfileEntity.new(@_client, opts)
  end
  ...
  # @return [UserProfile, Hash] the current UserProfile data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of UserProfile fields)
  def match_get
  ...
  #
  # @param reqmatch [UserProfileLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.UserProfile.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [UserProfile, Hash] the loaded UserProfile; raises CloudsmithError on failure
