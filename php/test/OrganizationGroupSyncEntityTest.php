...
class OrganizationGroupSyncEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->OrganizationGroupSync(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->OrganizationGroupSync(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->OrganizationGroupSync(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = organization_group_sync_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "organization_group_sync." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_GROUP_SYNC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $organization_group_sync_ref01_ent = $client->OrganizationGroupSync(null);
...
function organization_group_sync_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/organization_group_sync/OrganizationGroupSyncTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["organization_group_sync01", "organization_group_sync02", "organization_group_sync03", "org01", "org02", "org03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_ORGANIZATION_GROUP_SYNC_ENTID");
