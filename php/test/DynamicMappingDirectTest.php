...
class DynamicMappingDirectTest extends TestCase
{
    public function test_direct_list_dynamic_mapping(): void
    {
        $setup = dynamic_mapping_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-dynamic_mapping", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["org01", "provider_setting01"] as $_liveKey) {
    ...
    public function test_direct_load_dynamic_mapping(): void
    {
        $setup = dynamic_mapping_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-dynamic_mapping", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
