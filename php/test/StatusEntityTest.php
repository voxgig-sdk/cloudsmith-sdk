...
class StatusEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_STATUS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function status_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["status01", "status02", "status03", "org01", "org02", "org03", "package01", "package02", "package03", "repo01", "repo02", "repo03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_STATUS_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_STATUS_ENTID"]);
