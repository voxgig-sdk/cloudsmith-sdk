...
class FtCtrl extends CloudsmithControl
{
...
class FtClient
{
    ...
    private CloudsmithUtility $utility;

    public function __construct(CloudsmithUtility $utility, array $options)
    {
    ...
    public function get_utility(): CloudsmithUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CloudsmithContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CloudsmithContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CloudsmithContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CloudsmithConfig::make_feature($name);
    ...
    private static function buildUrl(CloudsmithSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CloudsmithError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CloudsmithResponse($fetched);
        }
    ...
    private function fail(CloudsmithContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CloudsmithContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
