...
class RepositoryTokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->RepositoryToken(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->RepositoryToken(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->RepositoryToken(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = repository_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "repository_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_REPOSITORY_TOKEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $repository_token_ref01_ent = $client->RepositoryToken(null);
...
function repository_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/repository_token/RepositoryTokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["repository_token01", "repository_token02", "repository_token03", "entitlement01", "entitlement02", "entitlement03", "owner01", "repo01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_REPOSITORY_TOKEN_ENTID");
