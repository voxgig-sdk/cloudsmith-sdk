...
class OrganizationInviteEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->OrganizationInvite(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->OrganizationInvite(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->OrganizationInvite(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = organization_invite_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "organization_invite." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $organization_invite_ref01_ent = $client->OrganizationInvite(null);
        ...
        $organization_invite_ref01_markdef_up0_value = "Mark01-organization_invite_ref01_" . $setup["now"];
...
function organization_invite_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/organization_invite/OrganizationInviteTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["organization_invite01", "organization_invite02", "organization_invite03", "org01", "org02", "org03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_ORGANIZATION_INVITE_ENTID");
