...
class DebEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Deb(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Deb(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Deb(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = deb_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "deb." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_DEB_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $deb_ref01_ent = $client->Deb(null);
...
function deb_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/deb/DebTestData.json';
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_DEB_ENTID");
