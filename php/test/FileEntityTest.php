...
class FileEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_FILE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function file_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["file01", "file02", "file03", "owner01", "repo01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_FILE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_FILE_ENTID"]);
