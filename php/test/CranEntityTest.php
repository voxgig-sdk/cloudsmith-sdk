...
class CranEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Cran(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Cran(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Cran(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = cran_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "cran." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_CRAN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $cran_ref01_ent = $client->Cran(null);
        $cran_ref01_data = Helpers::to_map(Vs::getprop(
        ...
        $this->assertNotNull($cran_ref01_data["id"]);
        ...
        $cran_ref01_markdef_up0_name = "auth_mode";
        $cran_ref01_markdef_up0_value = "Mark01-cran_ref01_" . $setup["now"];
        ...
        $this->assertEquals($cran_ref01_resdata_up0["id"], $cran_ref01_data_up0_up["id"]);
        ...
        $cran_ref01_data_dt0_loaded = $cran_ref01_ent->load($cran_ref01_match_dt0, null);
        $cran_ref01_data_dt0_load_result = Helpers::to_map(is_object($cran_ref01_data_dt0_loaded) && method_exists($cran_ref01_data_dt0_loaded, 'data_get') ? $cran_ref01_data_dt0_loaded->data_get() : $cran_ref01_data_dt0_loaded);
        $this->assertNotNull($cran_ref01_data_dt0_load_result);
        $this->assertEquals($cran_ref01_data_dt0_load_result["id"], $cran_ref01_data["id"]);
...
function cran_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/cran/CranTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["cran01", "cran02", "cran03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_CRAN_ENTID");
