...
class PackageFileUploadEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->PackageFileUpload(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = package_file_upload_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "package_file_upload." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_PACKAGE_FILE_UPLOAD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $package_file_upload_ref01_ent = $client->PackageFileUpload(null);
...
function package_file_upload_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/package_file_upload/PackageFileUploadTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["package_file_upload01", "package_file_upload02", "package_file_upload03", "file01", "file02", "file03", "identifier01", "owner01", "repo01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_PACKAGE_FILE_UPLOAD_ENTID");
