...
class TeamEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_TEAM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function team_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["team01", "team02", "team03", "org01", "org02", "org03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_TEAM_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_TEAM_ENTID"]);
