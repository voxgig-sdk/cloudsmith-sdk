...
class ConanEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_CONAN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function conan_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["conan01", "conan02", "conan03", "package01", "package02", "package03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_CONAN_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_CONAN_ENTID"]);
