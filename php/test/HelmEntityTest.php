...
class HelmEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Helm(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Helm(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Helm(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = helm_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "helm." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_HELM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $helm_ref01_ent = $client->Helm(null);
        ...
        $helm_ref01_markdef_up0_value = "Mark01-helm_ref01_" . $setup["now"];
        ...
        $helm_ref01_resdata_up0 = Helpers::to_map($helm_ref01_resdata_up0_result);
...
function helm_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/helm/HelmTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["helm01", "helm02", "helm03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_HELM_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_HELM_ENTID"]);
