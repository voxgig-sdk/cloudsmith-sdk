...
class NugetEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Nuget(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Nuget(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_NUGET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function nuget_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["nuget01", "nuget02", "nuget03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_NUGET_ENTID");
