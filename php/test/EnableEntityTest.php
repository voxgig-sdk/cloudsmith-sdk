...
class EnableEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ENABLE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function enable_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["enable01", "enable02", "enable03", "entitlement01", "entitlement02", "entitlement03", "org01", "org02", "org03", "repo01", "repo02", "repo03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_ENABLE_ENTID");
