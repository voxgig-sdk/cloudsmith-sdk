...
class CondaEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Conda(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Conda(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Conda(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = conda_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "conda." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_CONDA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $conda_ref01_ent = $client->Conda(null);
        ...
        $this->assertNotNull($conda_ref01_data["id"]);
        ...
        $this->assertEquals($conda_ref01_resdata_up0["id"], $conda_ref01_data_up0_up["id"]);
        ...
        $conda_ref01_data_dt0_loaded = $conda_ref01_ent->load($conda_ref01_match_dt0, null);
        $conda_ref01_data_dt0_load_result = Helpers::to_map(is_object($conda_ref01_data_dt0_loaded) && method_exists($conda_ref01_data_dt0_loaded, 'data_get') ? $conda_ref01_data_dt0_loaded->data_get() : $conda_ref01_data_dt0_loaded);
        $this->assertNotNull($conda_ref01_data_dt0_load_result);
        $this->assertEquals($conda_ref01_data_dt0_load_result["id"], $conda_ref01_data["id"]);
...
function conda_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/conda/CondaTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["conda01", "conda02", "conda03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_CONDA_ENTID");
