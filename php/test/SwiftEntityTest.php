...
class SwiftEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Swift(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Swift(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Swift(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = swift_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "swift." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_SWIFT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $swift_ref01_ent = $client->Swift(null);
        ...
        $this->assertNotNull($swift_ref01_data["id"]);
        ...
        $swift_ref01_markdef_up0_value = "Mark01-swift_ref01_" . $setup["now"];
        ...
        $this->assertEquals($swift_ref01_resdata_up0["id"], $swift_ref01_data_up0_up["id"]);
        ...
        $swift_ref01_data_dt0_loaded = $swift_ref01_ent->load($swift_ref01_match_dt0, null);
        $swift_ref01_data_dt0_load_result = Helpers::to_map(is_object($swift_ref01_data_dt0_loaded) && method_exists($swift_ref01_data_dt0_loaded, 'data_get') ? $swift_ref01_data_dt0_loaded->data_get() : $swift_ref01_data_dt0_loaded);
        $this->assertNotNull($swift_ref01_data_dt0_load_result);
        $this->assertEquals($swift_ref01_data_dt0_load_result["id"], $swift_ref01_data["id"]);

    }
}

function swift_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/swift/SwiftTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["swift01", "swift02", "swift03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_SWIFT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_SWIFT_ENTID"]);
