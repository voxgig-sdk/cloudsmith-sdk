...
class ProviderSettingsWriteEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->ProviderSettingsWrite(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = provider_settings_write_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "provider_settings_write." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_PROVIDER_SETTINGS_WRITE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $provider_settings_write_ref01_ent = $client->ProviderSettingsWrite(null);
...
function provider_settings_write_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/provider_settings_write/ProviderSettingsWriteTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["provider_settings_write01", "provider_settings_write02", "provider_settings_write03", "org01", "org02", "org03", "openid_connect01", "openid_connect02", "openid_connect03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_PROVIDER_SETTINGS_WRITE_ENTID");
