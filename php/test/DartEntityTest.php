...
class DartEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Dart(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Dart(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Dart(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = dart_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "dart." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_DART_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $dart_ref01_ent = $client->Dart(null);
...
function dart_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/dart/DartTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["dart01", "dart02", "dart03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_DART_ENTID");
