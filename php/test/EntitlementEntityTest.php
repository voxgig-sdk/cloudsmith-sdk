...
class EntitlementEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Entitlement(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = entitlement_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "entitlement." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_ENTITLEMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $entitlement_ref01_ent = $client->Entitlement(null);
...
function entitlement_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/entitlement/EntitlementTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["entitlement01", "entitlement02", "entitlement03", "identifier01", "owner01", "repo01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_ENTITLEMENT_ENTID");
