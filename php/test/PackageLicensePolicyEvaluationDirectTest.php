...
class PackageLicensePolicyEvaluationDirectTest extends TestCase
{
    public function test_direct_list_package_license_policy_evaluation(): void
    {
        $setup = package_license_policy_evaluation_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-package_license_policy_evaluation", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["org01", "policy_slug_perm01"] as $_liveKey) {
    ...
    public function test_direct_load_package_license_policy_evaluation(): void
    {
        $setup = package_license_policy_evaluation_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-package_license_policy_evaluation", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
