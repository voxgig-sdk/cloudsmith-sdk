...
class Gon8EntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Gon8(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = gon8_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "gon8." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $gon8_ref01_data = null;
        if (count($gon8_ref01_data_raw) > 0) {
            $gon8_ref01_data = Helpers::to_map($gon8_ref01_data_raw[0][1]);
        }

        // UPDATE
        $gon8_ref01_ent = $client->Gon8(null);
        ...
        $gon8_ref01_resdata_up0_result = $gon8_ref01_ent->update($gon8_ref01_data_up0_up, null);
        $gon8_ref01_resdata_up0 = Helpers::to_map($gon8_ref01_resdata_up0_result);
        $this->assertNotNull($gon8_ref01_resdata_up0);
        $this->assertEquals($gon8_ref01_resdata_up0[$gon8_ref01_markdef_up0_name], $gon8_ref01_markdef_up0_value);
...
function gon8_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/gon8/Gon8TestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["gon801", "gon802", "gon803", "repo01", "repo02", "repo03", "go01", "go02", "go03", "identifier01", "owner01"] as $k) {
