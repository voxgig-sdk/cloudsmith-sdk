...
class Gon3EntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Gon3(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Gon3(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Gon3(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = gon3_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "gon3." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $gon3_ref01_data = null;
        if (count($gon3_ref01_data_raw) > 0) {
            $gon3_ref01_data = Helpers::to_map($gon3_ref01_data_raw[0][1]);
        }

        // LIST
        $gon3_ref01_ent = $client->Gon3(null);
        $gon3_ref01_match = [
            "identifier" => $setup["idmap"]["identifier01"],
            "owner" => $setup["idmap"]["owner01"],
        ];

        $gon3_ref01_list_result = $gon3_ref01_ent->list($gon3_ref01_match, null);
        $this->assertIsArray($gon3_ref01_list_result);
...
function gon3_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/gon3/Gon3TestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["gon301", "gon302", "gon303", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
