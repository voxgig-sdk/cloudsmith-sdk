...
class HexEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CloudsmithSDK::test(null, null);
        $ent = $testsdk->Hex(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Hex(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Hex(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = hex_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "hex." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_HEX_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $hex_ref01_ent = $client->Hex(null);
...
function hex_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/hex/HexTestData.json';
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_HEX_ENTID");
