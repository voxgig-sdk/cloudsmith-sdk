...
class GroupEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_GROUP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function group_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["group01", "group02", "group03", "package01", "package02", "package03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_GROUP_ENTID");
