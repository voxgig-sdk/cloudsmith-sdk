...
class NpmEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Npm(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Npm(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_NPM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($npm_ref01_data["id"]);
        ...
        $npm_ref01_data_dt0_loaded = $npm_ref01_ent->load($npm_ref01_match_dt0, null);
        $npm_ref01_data_dt0_load_result = Helpers::to_map(is_object($npm_ref01_data_dt0_loaded) && method_exists($npm_ref01_data_dt0_loaded, 'data_get') ? $npm_ref01_data_dt0_loaded->data_get() : $npm_ref01_data_dt0_loaded);
        $this->assertNotNull($npm_ref01_data_dt0_load_result);
        $this->assertEquals($npm_ref01_data_dt0_load_result["id"], $npm_ref01_data["id"]);
...
function npm_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["npm01", "npm02", "npm03", "package01", "package02", "package03", "repo01", "repo02", "repo03", "identifier01", "owner01"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_NPM_ENTID");
