...
class MemberEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_MEMBER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function member_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["member01", "member02", "member03", "org01", "org02", "org03"] as $k) {
    ...
    $entid_env_raw = getenv("CLOUDSMITH_TEST_MEMBER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CLOUDSMITH_TEST_MEMBER_ENTID"]);
