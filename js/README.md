# Cloudsmith JavaScript SDK



The JavaScript SDK for the Cloudsmith API — an entity-oriented client with full async/await support.
...
```js
const { CloudsmithSDK } = require('@voxgig-sdk/cloudsmith-js')
...
// vulnerability is a bare entity populated with mock response data
...
```ts
const gons = await client.Gon().list()
```
...
cloudsmith/
├── src/
│   ├── CloudsmithSDK.js        # Main SDK class
