...
describe('CargoEntity', async () => {
  ...
  test('basic', async () => {
    ...
    assert(cargo_ref01_resdata_up0[cargo_ref01_markdef_up0.name] === cargo_ref01_markdef_up0.value)


    // LOAD
    const cargo_ref01_match_dt0 = {}
    const cargo_ref01_data_dt0 = await cargo_ref01_ent.load(cargo_ref01_match_dt0)
    assert(null != cargo_ref01_data_dt0)
...
function basicSetup(extra) {
  ...
  const entityDataFile =
    Path.resolve(__dirname,
      '../../../../.sdk/test/entity/cargo/CargoTestData.json')
  ...
  let idmap = transform(
    ['cargo01','cargo02','cargo03','package01','package02','package03','repo01','repo02','repo03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
