...
describe('RepositoryTokenEntity', async () => {
  ...
  test('basic', async () => {
    ...
    assert(repository_token_ref01_resdata_up0[repository_token_ref01_markdef_up0.name] === repository_token_ref01_markdef_up0.value)


    // LOAD
    const repository_token_ref01_match_dt0 = {}
    const repository_token_ref01_data_dt0 = await repository_token_ref01_ent.load(repository_token_ref01_match_dt0)
    assert(null != repository_token_ref01_data_dt0)
...
function basicSetup(extra) {
  ...
  let idmap = transform(
    ['repository_token01','repository_token02','repository_token03','entitlement01','entitlement02','entitlement03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
