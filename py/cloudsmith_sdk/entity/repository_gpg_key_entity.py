...
class RepositoryGpgKeyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RepositoryGpgKeyEntity(self._client, opts)
    ...
    def data_get(self) -> RepositoryGpgKey:
    ...
    def match_get(self) -> RepositoryGpgKey:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RepositoryGpgKey:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RepositoryGpgKey().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: RepositoryGpgKeyCreateData, ctrl=None) -> RepositoryGpgKey:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
