...
class RepositoryGeoIpRuleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RepositoryGeoIpRuleEntity(self._client, opts)
    ...
    def data_get(self) -> RepositoryGeoIpRule:
    ...
    def match_get(self) -> RepositoryGeoIpRule:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RepositoryGeoIpRule:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RepositoryGeoIpRule().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: RepositoryGeoIpRuleUpdateData, ctrl=None) -> RepositoryGeoIpRule:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
