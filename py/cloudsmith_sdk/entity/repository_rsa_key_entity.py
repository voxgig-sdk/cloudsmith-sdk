...
class RepositoryRsaKeyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RepositoryRsaKeyEntity(self._client, opts)
    ...
    def data_get(self) -> RepositoryRsaKey:
    ...
    def match_get(self) -> RepositoryRsaKey:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RepositoryRsaKey:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RepositoryRsaKey().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: RepositoryRsaKeyCreateData, ctrl=None) -> RepositoryRsaKey:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
