...
class ProviderSettingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProviderSettingEntity(self._client, opts)
    ...
    def data_get(self) -> ProviderSetting:
    ...
    def match_get(self) -> ProviderSetting:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ProviderSetting:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ProviderSetting().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ProviderSetting]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ProviderSetting().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
