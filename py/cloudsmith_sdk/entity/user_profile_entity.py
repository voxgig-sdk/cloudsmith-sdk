...
class UserProfileEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UserProfileEntity(self._client, opts)
    ...
    def match_get(self) -> UserProfile:
    ...
    def load(self, reqmatch=None, ctrl=None) -> UserProfile:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.UserProfile().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
