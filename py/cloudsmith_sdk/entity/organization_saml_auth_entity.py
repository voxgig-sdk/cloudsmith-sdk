...
class OrganizationSamlAuthEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OrganizationSamlAuthEntity(self._client, opts)
    ...
    def data_get(self) -> OrganizationSamlAuth:
    ...
    def match_get(self) -> OrganizationSamlAuth:
    ...
    def load(self, reqmatch=None, ctrl=None) -> OrganizationSamlAuth:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OrganizationSamlAuth().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: OrganizationSamlAuthUpdateData, ctrl=None) -> OrganizationSamlAuth:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
