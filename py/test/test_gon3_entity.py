...
class TestGon3Entity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Gon3(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Gon3(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _gon3_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "gon3." + _op, "live" if _live else "unit")
            if _skip:
        ...
        gon3_ref01_data = None
        if len(gon3_ref01_data_raw) > 0:
            gon3_ref01_data = helpers.to_map(gon3_ref01_data_raw[0][1])

        # LIST
        gon3_ref01_ent = client.Gon3(None)
        gon3_ref01_match = {
            "identifier": setup["idmap"]["identifier01"],
            "owner": setup["idmap"]["owner01"],
        }

        gon3_ref01_list_result = gon3_ref01_ent.list(gon3_ref01_match, None)
        assert isinstance(gon3_ref01_list_result, list)



def _gon3_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/gon3/Gon3TestData.json")
