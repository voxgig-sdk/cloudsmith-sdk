...
class TestPackageFilePartsUploadDirect:

    def test_should_direct_load_package_file_parts_upload(self):
        setup = _package_file_parts_upload_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-package_file_parts_upload", "live" if setup["live"] else "unit")
        if _skip:
...
def _package_file_parts_upload_direct_setup(mockres):
