...
class TestOrganizationTeamMemberDirect:

    def test_should_direct_list_organization_team_member(self):
        setup = _organization_team_member_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_team_member", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["org01", "team01"]:
...
def _organization_team_member_direct_setup(mockres):
