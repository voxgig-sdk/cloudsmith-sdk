...
class TestRepositoryAuditLogDirect:

    def test_should_direct_list_repository_audit_log(self):
        setup = _repository_audit_log_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-repository_audit_log", "live" if setup["live"] else "unit")
        if _skip:
...
def _repository_audit_log_direct_setup(mockres):
