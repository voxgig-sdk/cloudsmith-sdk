...
class TestGon8Entity:

    def test_should_create_instance(self):
        testsdk = CloudsmithSDK.test(None, None)
        ent = testsdk.Gon8(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _gon8_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "gon8." + _op, "live" if _live else "unit")
            if _skip:
        ...
        gon8_ref01_data = None
        if len(gon8_ref01_data_raw) > 0:
            gon8_ref01_data = helpers.to_map(gon8_ref01_data_raw[0][1])

        # UPDATE
        gon8_ref01_ent = client.Gon8(None)
        ...
        gon8_ref01_markdef_up0_value = "Mark01-gon8_ref01_" + str(setup["now"])
        gon8_ref01_data_up0_up[gon8_ref01_markdef_up0_name] = gon8_ref01_markdef_up0_value

        gon8_ref01_resdata_up0 = helpers.to_map(gon8_ref01_ent.update(gon8_ref01_data_up0_up, None))
        assert gon8_ref01_resdata_up0 is not None
        assert gon8_ref01_resdata_up0[gon8_ref01_markdef_up0_name] == gon8_ref01_markdef_up0_value



def _gon8_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/gon8/Gon8TestData.json")
