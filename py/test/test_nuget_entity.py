...
class TestNugetEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Nuget(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Nuget(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_NUGET_ENTID JSON to run live")
        client = setup["client"]
        ...
        nuget_ref01_data = helpers.to_map(nuget_ref01_ent.create(nuget_ref01_data, None))
        ...
        nuget_ref01_markdef_up0_value = "Mark01-nuget_ref01_" + str(setup["now"])
...
def _nuget_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_NUGET_ENTID"))
