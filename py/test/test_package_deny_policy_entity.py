...
class TestPackageDenyPolicyEntity:

    def test_should_create_instance(self):
        testsdk = CloudsmithSDK.test(None, None)
        ent = testsdk.PackageDenyPolicy(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.PackageDenyPolicy(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.PackageDenyPolicy(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _package_deny_policy_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "package_deny_policy." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_PACKAGE_DENY_POLICY_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        package_deny_policy_ref01_ent = client.PackageDenyPolicy(None)
        ...
        assert package_deny_policy_ref01_data["id"] is not None
        ...
        package_deny_policy_ref01_markdef_up0_name = "description"
        ...
        assert package_deny_policy_ref01_resdata_up0["id"] == package_deny_policy_ref01_data_up0_up["id"]
        ...
        package_deny_policy_ref01_data_dt0_loaded = package_deny_policy_ref01_ent.load(package_deny_policy_ref01_match_dt0, None)
        package_deny_policy_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(package_deny_policy_ref01_data_dt0_loaded))
        assert package_deny_policy_ref01_data_dt0_load_result is not None
        assert package_deny_policy_ref01_data_dt0_load_result["id"] == package_deny_policy_ref01_data["id"]



def _package_deny_policy_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/package_deny_policy/PackageDenyPolicyTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CLOUDSMITH_TEST_PACKAGE_DENY_POLICY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_PACKAGE_DENY_POLICY_ENTID"))
