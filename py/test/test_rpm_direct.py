...
class TestRpmDirect:
    ...
    def test_should_direct_list_rpm(self):
        setup = _rpm_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-rpm", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_rpm(self):
        setup = _rpm_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-rpm", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
