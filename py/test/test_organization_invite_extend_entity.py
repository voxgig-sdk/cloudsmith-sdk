...
class TestOrganizationInviteExtendEntity:

    def test_should_create_instance(self):
        testsdk = CloudsmithSDK.test(None, None)
        ent = testsdk.OrganizationInviteExtend(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _organization_invite_extend_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "organization_invite_extend." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_ORGANIZATION_INVITE_EXTEND_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        organization_invite_extend_ref01_ent = client.OrganizationInviteExtend(None)
...
def _organization_invite_extend_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/organization_invite_extend/OrganizationInviteExtendTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CLOUDSMITH_TEST_ORGANIZATION_INVITE_EXTEND_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_ORGANIZATION_INVITE_EXTEND_ENTID"))
