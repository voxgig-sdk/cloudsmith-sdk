...
class TestPackageEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Package(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Package(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _package_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "package." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_PACKAGE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        package_ref01_ent = client.Package(None)
...
def _package_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/package/PackageTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_PACKAGE_ENTID"))
