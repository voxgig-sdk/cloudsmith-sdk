...
class TestNugetDirect:

    def test_should_direct_list_nuget(self):
        setup = _nuget_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-nuget", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_nuget(self):
        setup = _nuget_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-nuget", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _nuget_direct_setup(mockres):
