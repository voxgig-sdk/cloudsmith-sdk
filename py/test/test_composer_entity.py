...
class TestComposerEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Composer(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Composer(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_COMPOSER_ENTID JSON to run live")
        client = setup["client"]
        ...
        composer_ref01_markdef_up0_value = "Mark01-composer_ref01_" + str(setup["now"])
        composer_ref01_data_up0_up[composer_ref01_markdef_up0_name] = composer_ref01_markdef_up0_value

        composer_ref01_resdata_up0 = helpers.to_map(composer_ref01_ent.update(composer_ref01_data_up0_up, None))
...
def _composer_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_COMPOSER_ENTID"))
