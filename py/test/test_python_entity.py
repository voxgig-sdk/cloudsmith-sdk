...
class TestPythonEntity:

    def test_should_create_instance(self):
        testsdk = CloudsmithSDK.test(None, None)
        ent = testsdk.Python(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Python(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Python(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _python_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "python." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_PYTHON_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        python_ref01_ent = client.Python(None)
        ...
        python_ref01_data = helpers.to_map(python_ref01_ent.create(python_ref01_data, None))
        ...
        python_ref01_markdef_up0_value = "Mark01-python_ref01_" + str(setup["now"])
        python_ref01_data_up0_up[python_ref01_markdef_up0_name] = python_ref01_markdef_up0_value

        python_ref01_resdata_up0 = helpers.to_map(python_ref01_ent.update(python_ref01_data_up0_up, None))
...
def _python_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/python/PythonTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_PYTHON_ENTID"))
