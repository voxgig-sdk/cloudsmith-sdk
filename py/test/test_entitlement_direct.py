...
class TestEntitlementDirect:

    def test_should_direct_load_entitlement(self):
        setup = _entitlement_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-entitlement", "live" if setup["live"] else "unit")
        if _skip:
...
def _entitlement_direct_setup(mockres):
