...
class TestPackageLicensePolicyEvaluationDirect:

    def test_should_direct_list_package_license_policy_evaluation(self):
        setup = _package_license_policy_evaluation_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-package_license_policy_evaluation", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["org01", "policy_slug_perm01"]:
    ...
    def test_should_direct_load_package_license_policy_evaluation(self):
        setup = _package_license_policy_evaluation_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-package_license_policy_evaluation", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _package_license_policy_evaluation_direct_setup(mockres):
