...
class TestRepositoryX509EcdsaCertificateDirect:

    def test_should_direct_load_repository_x509_ecdsa_certificate(self):
        setup = _repository_x509_ecdsa_certificate_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-repository_x509_ecdsa_certificate", "live" if setup["live"] else "unit")
        if _skip:
...
def _repository_x509_ecdsa_certificate_direct_setup(mockres):
