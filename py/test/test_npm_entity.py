...
class TestNpmEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Npm(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Npm(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_NPM_ENTID JSON to run live")
        client = setup["client"]
        ...
        npm_ref01_data = helpers.to_map(npm_ref01_ent.create(npm_ref01_data, None))
        ...
        npm_ref01_markdef_up0_value = "Mark01-npm_ref01_" + str(setup["now"])
...
def _npm_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_NPM_ENTID"))
