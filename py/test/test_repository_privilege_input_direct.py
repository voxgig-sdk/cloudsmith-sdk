...
class TestRepositoryPrivilegeInputDirect:

    def test_should_direct_list_repository_privilege_input(self):
        setup = _repository_privilege_input_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-repository_privilege_input", "live" if setup["live"] else "unit")
        if _skip:
...
def _repository_privilege_input_direct_setup(mockres):
