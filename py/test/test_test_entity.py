    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_TEST_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        test_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.test")))
        test_ref01_data = None
        if len(test_ref01_data_raw) > 0:
...
def _test_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_TEST_ENTID"))
