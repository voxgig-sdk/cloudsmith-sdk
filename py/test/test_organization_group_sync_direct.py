...
class TestOrganizationGroupSyncDirect:

    def test_should_direct_list_organization_group_sync(self):
        setup = _organization_group_sync_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_group_sync", "live" if setup["live"] else "unit")
        if _skip:
...
def _organization_group_sync_direct_setup(mockres):
