...
class TestCranEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Cran(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Cran(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _cran_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "cran." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_CRAN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        cran_ref01_ent = client.Cran(None)
        ...
        cran_ref01_data = helpers.to_map(cran_ref01_ent.create(cran_ref01_data, None))
        ...
        cran_ref01_markdef_up0_value = "Mark01-cran_ref01_" + str(setup["now"])
        cran_ref01_data_up0_up[cran_ref01_markdef_up0_name] = cran_ref01_markdef_up0_value

        cran_ref01_resdata_up0 = helpers.to_map(cran_ref01_ent.update(cran_ref01_data_up0_up, None))
...
def _cran_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/cran/CranTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_CRAN_ENTID"))
