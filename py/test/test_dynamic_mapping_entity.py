...
class TestDynamicMappingEntity:

    def test_should_create_instance(self):
        testsdk = CloudsmithSDK.test(None, None)
        ent = testsdk.DynamicMapping(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.DynamicMapping(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.DynamicMapping(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _dynamic_mapping_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "dynamic_mapping." + _op, "live" if _live else "unit")
            if _skip:
...
def _dynamic_mapping_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/dynamic_mapping/DynamicMappingTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_DYNAMIC_MAPPING_ENTID"))
