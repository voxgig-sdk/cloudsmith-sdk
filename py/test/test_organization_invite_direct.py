...
class TestOrganizationInviteDirect:

    def test_should_direct_list_organization_invite(self):
        setup = _organization_invite_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-organization_invite", "live" if setup["live"] else "unit")
        if _skip:
...
def _organization_invite_direct_setup(mockres):
