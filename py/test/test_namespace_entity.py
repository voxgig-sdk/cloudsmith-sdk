...
class TestNamespaceEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Namespace(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Namespace(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CLOUDSMITH_TEST_NAMESPACE_ENTID JSON to run live")
        client = setup["client"]
        ...
        namespace_ref01_list_result = namespace_ref01_ent.list(namespace_ref01_match, None)
...
def _namespace_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CLOUDSMITH_TEST_NAMESPACE_ENTID"))
