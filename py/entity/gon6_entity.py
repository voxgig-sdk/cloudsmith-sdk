...
class Gon6Entity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return Gon6Entity(self._client, opts)
    ...
    def match_get(self) -> Gon6:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> Gon6:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Gon6().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
