...
(0, node_test_1.describe)('RepositoryGeoIpRuleEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['update', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'repository_geo_ip_rule.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['repository_geo_ip_rule01', 'repository_geo_ip_rule02', 'repository_geo_ip_rule03', 'repo01', 'repo02', 'repo03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CLOUDSMITH_TEST_REPOSITORY_GEO_IP_RULE_ENTID'];
