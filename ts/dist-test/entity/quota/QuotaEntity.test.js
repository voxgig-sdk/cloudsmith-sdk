...
(0, node_test_1.describe)('QuotaEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set CLOUDSMITH_TEST_QUOTA_ENTID JSON to run live');
            return;
        }
        ...
        (0, node_assert_1.default)(quota_ref01_data_dt0.id === quota_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    let idmap = transform(['quota01', 'quota02', 'quota03', 'history01', 'history02', 'history03', 'oss01', 'oss02', 'oss03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CLOUDSMITH_TEST_QUOTA_ENTID'];
