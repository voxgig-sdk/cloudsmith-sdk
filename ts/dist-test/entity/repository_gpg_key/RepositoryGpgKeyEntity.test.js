...
(0, node_test_1.describe)('RepositoryGpgKeyEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'repository_gpg_key.' + op, live))
                return;
        }
...
function basicSetup(extra) {
    ...
    let idmap = transform(['repository_gpg_key01', 'repository_gpg_key02', 'repository_gpg_key03', 'repo01', 'repo02', 'repo03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['CLOUDSMITH_TEST_REPOSITORY_GPG_KEY_ENTID'];
