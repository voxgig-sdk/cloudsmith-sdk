...
(0, node_test_1.describe)('OrganizationMembershipDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-organization_membership', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-organization_membership', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["member01", "org01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.member ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.member = candidateId;
    ...
    (0, node_test_1.test)('direct-list-organization_membership', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-organization_membership', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CLOUDSMITH_TEST_ORGANIZATION_MEMBERSHIP_ENTID'];
...
//# sourceMappingURL=OrganizationMembershipDirect.test.js.map