...
(0, node_test_1.describe)('PackageDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-package', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-package', setup.live))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.owner ?? listArr[0]?.id;
            if (null == candidateId) {
    ...
    (0, node_test_1.test)('direct-list-package', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-package', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["owner01", "repo01"]))
            return;
        const { client, calls } = setup;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CLOUDSMITH_TEST_PACKAGE_ENTID'];
...
//# sourceMappingURL=PackageDirect.test.js.map