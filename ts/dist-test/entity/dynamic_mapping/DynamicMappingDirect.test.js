...
(0, node_test_1.describe)('DynamicMappingDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-dynamic_mapping', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-dynamic_mapping', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["openid_connect01", "org01"]))
            return;
    ...
    (0, node_test_1.test)('direct-list-dynamic_mapping', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-dynamic_mapping', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["org01", "provider_setting01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CLOUDSMITH_TEST_DYNAMIC_MAPPING_ENTID'];
...
//# sourceMappingURL=DynamicMappingDirect.test.js.map