...
describe('NamespaceAuditLogDirect', async () => {
  ...
  test('direct-load-namespace_audit_log', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-namespace_audit_log', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_NAMESPACE_AUDIT_LOG_ENTID']
