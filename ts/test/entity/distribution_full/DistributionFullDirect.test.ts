...
describe('DistributionFullDirect', async () => {
  ...
  test('direct-load-distribution_full', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-distribution_full', setup.live)) return
    if (skipIfMissingIds(t, setup, ["slug01"])) return
  ...
  test('direct-list-distribution_full', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-distribution_full', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_DISTRIBUTION_FULL_ENTID']
