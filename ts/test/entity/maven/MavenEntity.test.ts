...
describe('MavenEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'maven.' + op, live)) return
    }
    ...
    assert(null != maven_ref01_data.id)


    // LIST
    const maven_ref01_match: any = {}
    ...
    // UPDATE
    const maven_ref01_data_up0: any = {}
    ...
    assert(maven_ref01_data_dt0.id === maven_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/maven/MavenTestData.json')
  ...
  let idmap = transform(
    ['maven01','maven02','maven03','package01','package02','package03','repo01','repo02','repo03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
