...
describe('GonDirect', async () => {
  ...
  test('direct-load-gon', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-gon', setup.live)) return
    if (skipIfMissingIds(t, setup, ["identifier01","owner01","slug_perm01"])) return
  ...
  test('direct-list-gon', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-gon', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_GON_ENTID']
