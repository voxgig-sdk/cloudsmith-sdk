...
describe('PackageVersionBadgeDirect', async () => {
  ...
  test('direct-load-package_version_badge', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-package_version_badge', setup.live)) return
    if (skipIfMissingIds(t, setup, ["owner01","package_format01","package_identifier01","package_name01","package_version01","repo01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_PACKAGE_VERSION_BADGE_ENTID']
