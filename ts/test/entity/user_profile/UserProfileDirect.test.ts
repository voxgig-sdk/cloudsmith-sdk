...
describe('UserProfileDirect', async () => {
  ...
  test('direct-load-user_profile', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-user_profile', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_USER_PROFILE_ENTID']
