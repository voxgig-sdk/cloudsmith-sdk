...
describe('RepositoryX509RsaCertificateDirect', async () => {
  ...
  test('direct-load-repository_x509_rsa_certificate', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-repository_x509_rsa_certificate', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_REPOSITORY_X509_RSA_CERTIFICATE_ENTID']
