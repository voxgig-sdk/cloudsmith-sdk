...
describe('OrganizationPackageLicensePolicyDirect', async () => {
  ...
  test('direct-load-organization_package_license_policy', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-organization_package_license_policy', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01"])) return
  ...
  test('direct-list-organization_package_license_policy', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-organization_package_license_policy', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_ORGANIZATION_PACKAGE_LICENSE_POLICY_ENTID']
