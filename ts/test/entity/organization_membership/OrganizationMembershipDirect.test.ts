...
describe('OrganizationMembershipDirect', async () => {
  ...
  test('direct-load-organization_membership', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-organization_membership', setup.live)) return
    if (skipIfMissingIds(t, setup, ["member01","org01"])) return
  ...
  test('direct-list-organization_membership', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-organization_membership', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_ORGANIZATION_MEMBERSHIP_ENTID']
