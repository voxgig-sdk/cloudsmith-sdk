...
describe('RepositoryRsaKeyDirect', async () => {
  ...
  test('direct-load-repository_rsa_key', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-repository_rsa_key', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_REPOSITORY_RSA_KEY_ENTID']
