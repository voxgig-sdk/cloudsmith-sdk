...
describe('CranEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'cran.' + op, live)) return
    }
    ...
    assert(null != cran_ref01_data.id)


    // LIST
    const cran_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/cran/CranTestData.json')
  ...
  let idmap = transform(
    ['cran01','cran02','cran03','package01','package02','package03','repo01','repo02','repo03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
