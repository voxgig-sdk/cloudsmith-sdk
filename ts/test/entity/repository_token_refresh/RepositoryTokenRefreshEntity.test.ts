...
describe('RepositoryTokenRefreshEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'repository_token_refresh.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['repository_token_refresh01','repository_token_refresh02','repository_token_refresh03','entitlement01','entitlement02','entitlement03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['CLOUDSMITH_TEST_REPOSITORY_TOKEN_REFRESH_ENTID']
