...
describe('HexEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'hex.' + op, live)) return
    }
    ...
    assert(null != hex_ref01_data.id)


    // LIST
    const hex_ref01_match: any = {}
