...
describe('PythonEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'python.' + op, live)) return
    }
    ...
    assert(null != python_ref01_data.id)


    // LIST
    const python_ref01_match: any = {}
    ...
    // UPDATE
    const python_ref01_data_up0: any = {}
    ...
    assert(python_ref01_data_dt0.id === python_ref01_data.id)
