...
describe('OrganizationGroupSyncStatusDirect', async () => {
  ...
  test('direct-load-organization_group_sync_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-organization_group_sync_status', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_ORGANIZATION_GROUP_SYNC_STATUS_ENTID']
