...
describe('OrganizationTeamMemberDirect', async () => {
  ...
  test('direct-list-organization_team_member', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-organization_team_member', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01","team01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_ORGANIZATION_TEAM_MEMBER_ENTID']
