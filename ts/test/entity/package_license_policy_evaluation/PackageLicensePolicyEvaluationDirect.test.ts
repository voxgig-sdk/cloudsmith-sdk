...
describe('PackageLicensePolicyEvaluationDirect', async () => {
  ...
  test('direct-load-package_license_policy_evaluation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-package_license_policy_evaluation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["license_policy01","org01"])) return
  ...
  test('direct-list-package_license_policy_evaluation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-package_license_policy_evaluation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01","policy_slug_perm01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_PACKAGE_LICENSE_POLICY_EVALUATION_ENTID']
