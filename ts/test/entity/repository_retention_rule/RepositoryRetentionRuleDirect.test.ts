...
describe('RepositoryRetentionRuleDirect', async () => {
  ...
  test('direct-load-repository_retention_rule', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-repository_retention_rule', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_REPOSITORY_RETENTION_RULE_ENTID']
