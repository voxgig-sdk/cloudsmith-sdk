...
describe('RepositoryAuditLogDirect', async () => {
  ...
  test('direct-list-repository_audit_log', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-repository_audit_log', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_REPOSITORY_AUDIT_LOG_ENTID']
