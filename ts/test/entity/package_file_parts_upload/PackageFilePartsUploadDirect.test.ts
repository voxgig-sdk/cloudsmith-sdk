...
describe('PackageFilePartsUploadDirect', async () => {
  ...
  test('direct-load-package_file_parts_upload', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-package_file_parts_upload', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_PACKAGE_FILE_PARTS_UPLOAD_ENTID']
