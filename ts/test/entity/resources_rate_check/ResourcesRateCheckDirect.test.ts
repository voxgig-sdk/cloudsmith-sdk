...
describe('ResourcesRateCheckDirect', async () => {
  ...
  test('direct-load-resources_rate_check', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-resources_rate_check', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_RESOURCES_RATE_CHECK_ENTID']
