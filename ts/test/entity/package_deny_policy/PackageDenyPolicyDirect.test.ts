...
describe('PackageDenyPolicyDirect', async () => {
  ...
  test('direct-load-package_deny_policy', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-package_deny_policy', setup.live)) return
  ...
  test('direct-list-package_deny_policy', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-package_deny_policy', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_PACKAGE_DENY_POLICY_ENTID']
