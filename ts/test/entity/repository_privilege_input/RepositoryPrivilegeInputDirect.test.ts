...
describe('RepositoryPrivilegeInputDirect', async () => {
  ...
  test('direct-list-repository_privilege_input', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-repository_privilege_input', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_REPOSITORY_PRIVILEGE_INPUT_ENTID']
