...
describe('ProviderSettingDirect', async () => {
  ...
  test('direct-load-provider_setting', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-provider_setting', setup.live)) return
    if (skipIfMissingIds(t, setup, ["org01","slug_perm01"])) return
  ...
  test('direct-list-provider_setting', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-provider_setting', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CLOUDSMITH_TEST_PROVIDER_SETTING_ENTID']
